...
{
    double ra, dec, spd
    double mfp, mjp, mv, mnp, mu, mb, mr, mi, mj, mh, mk
    double efp, ejp, ev, enp, eu, eb, er, ei, ej, eh, ek
    int fstat, fvar, fmult
    int i, j;
    char root[] = "/pool/megascr1/mink/gsc232";
    char dir[256];
	...
	if (access (dir, F_OK&W_OK)) {
	    if (mkdir (dir, 00777))
		fprintf (stderr, "GSCATE: Cannot make directory %s\n", dir);
	    }
	}
	...
	moveb (line, ra, 8, 26, 0);
	moveb (line, dec, 8, 34, 0);
	...
	if (strcmp (file, file0)) {
	    /* if (nz > 1)
		printf (" GSCATE: %7d stars written to %s\n", nz, file0); */
	    if (nz == 1) {
	/* 	printf (" GSCATE: %7d stars written to %s", nz, file0);
		...
		if (outfile == NULL) {
		    printf ("GSCATE: Cannot write to file %s\n", file);
		    continue;
		    }
		}
	    strcpy (file0, file);
	...
	/* ID */
	moveb (line, id, 11, 4, 0);

	/* Fpg, Jpg, V, Npg magnitudes and error */
	moveb (line, mfp, 4, 70, 0);
	moveb (line, mfpe, 4, 74, 0);
	moveb (line, mjp, 4, 80, 0);
	moveb (line, mjpe, 4, 84, 0);
	moveb (line, mv, 4, 90, 0);
	moveb (line, mve, 4, 94, 0);
	moveb (line, mnp, 4, 100, 0);
	moveb (line, mnpe, 4, 104, 0);

	/* U, B, R, I magnitudes and errors

	/* J, H, K magnitudes and errors */
	ftgetr4 (line, "Jmag")
	mj = atof (token);
...
/* Mar 11 2005	New program, based on tmcate.c
