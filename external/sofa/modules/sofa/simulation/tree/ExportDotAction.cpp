...
ExportDotAction::ExportDotAction(std::ostream* out)
: out(out),
  showNode(true),
  showObject(true),
...
ExportDotAction::~ExportDotAction()
{
	*out << "}" << std::endl;
}

/// Test if a node should be displayed
bool ExportDotAction::display(GNode* node, const char **color)
{
...
bool ExportDotAction::display(core::objectmodel::BaseObject* obj, const char **color)
{
	using namespace Colors;
	const char* c = NULL;
...
std::string ExportDotAction::getParentName(core::objectmodel::BaseObject* obj)
{
	...
	if (dynamic_cast<core::BaseMapping*>(obj))
		return "";
	if (!node->collisionPipeline.empty() && display(node->collisionPipeline[0]) &&
		(dynamic_cast<core::componentmodel::collision::Intersection*>(obj) ||
		 dynamic_cast<core::componentmodel::collision::Detection*>(obj) ||
		 dynamic_cast<core::componentmodel::collision::ContactManager*>(obj) ||
		 dynamic_cast<core::componentmodel::collision::CollisionGroupManager*>(obj)))
		 return getName(node->collisionPipeline[0]);
	if (node->mechanicalModel!=obj && node->solver!=obj && display(node->mechanicalModel))
		return getName(node->mechanicalModel);
	if (node->solver!=obj && display(node->solver))
		return getName(node->solver);
	if ((node->mechanicalModel==obj || node->solver==obj) && node->mechanicalMapping==NULL && node->parent!=NULL && display(node->parent->solver))
		return getName(node->parent->solver);
	return "";
}

/// Compute the name of a given node or object
std::string ExportDotAction::getName(core::objectmodel::Base* o, std::string prefix)
{
	if (names.count(o)>0)
		return names[o];
...
std::string ExportDotAction::getName(GNode* node)
{
	return getName(node, "n_");
}

/// Compute the name of a given object
std::string ExportDotAction::getName(core::objectmodel::BaseObject* obj)
{
	return getName(obj, "o_");
}

void ExportDotAction::processObject(GNode* /*node*/, core::objectmodel::BaseObject* obj)
{
...
Action::Result ExportDotAction::processNodeTopDown(GNode* node)
{
	const char* color=NULL;
...
void ExportDotAction::processNodeBottomUp(GNode* /*node*/)
{
}
