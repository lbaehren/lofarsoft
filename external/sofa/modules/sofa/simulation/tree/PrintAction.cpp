...
template<class T>
void PrintAction::processObject(T obj)
{
	std::cout << ' ' << obj->getName() << '(' << sofa::helper::gettypename(typeid(*obj)) << ')';
}

template<class Seq>
void PrintAction::processObjects(Seq& list, const char* name)
{
	...
	//for_each<PrintAction, Seq, typename Seq::value_type>(this, list, &PrintAction::processObject<typename Seq::value_type>);
...
Action::Result PrintAction::processNodeTopDown(GNode* node)
{
	for (int i=0;i<level;i++)
		std::cout << "| ";
	std::cout << "+-";
	...
	++level;
	processObjects(node->mechanicalModel,"MechanicalState");
...
void PrintAction::processNodeBottomUp(GNode* /*node*/)
{
	--level;
}
