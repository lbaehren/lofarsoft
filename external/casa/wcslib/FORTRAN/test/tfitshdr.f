...
*   $Id: tfitshdr.f,v 4.2 2005/09/21 13:37:53 cal103 Exp $
*=======================================================================
...
*
*   WCSHDR is called first to extract all WCS-related cards from the
*   input header before passing it on to FITSHDR.
*
*-----------------------------------------------------------------------
      LOGICAL  GOTEND
      INTEGER  CTRL, I, IERR, IVAL(8), J, K, KEYNO, KEYS, KEYTYP, KTYP,
     :         NC, NCARDS, NKEYID, NREJECT, NWCS, RELAX, STATUS, ULEN,
     :         WCSP
      DOUBLE PRECISION FVAL(2)
      CHARACTER CARD*80, CVAL*72, HEADER*288001, KEYWRD*12, INFILE*9,
     ;         TEXT*84
...
*              An END card was read, but read the rest of the block.
 ...
 60   CLOSE (UNIT=1)
      ...
      HEADER(K:K) = CHAR (0)
      WRITE (*, 70) NCARDS
 70   FORMAT ('Found',I4,' header cards.')
...
*     Specific keywords to be located or culled.
      ...
      IF (NKEYID.GT.0) THEN
         WRITE (*, '(/,A)') 'The following cards will not be listed:'
         DO 120 I = 0, NKEYID-1
...
*     Parse the header.
      IERR = FITSHDR (HEADER, NCARDS, NKEYID, KEYIDS, NREJECT, KEYS)
      IF (IERR.NE.0) THEN
         WRITE (*, 130) IERR
...
*     Report the results.
      WRITE (*, 140) NCARDS, NREJECT
 140  FORMAT(/,I3,' header cards parsed by FITSHDR,',I3,' rejected:',/)
      DO 200 I = 0, NCARDS-1
