...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt (argc, argv, "hf:i:cb")) != -1) //: after var ->Waits for input
    switch (c)
      {
      case 'f':
	...
	cout << " -c	enable calibration (default: disabled)\n";
	cout << " -r	enable calibration with reference source (default: lab measurement)\n";
...
///////////////////////////////
  ...
  int xbinTime  = (int)(((int)xStopTime-(int)xStartTime)/180.0);
  ...
  for(int i=START; i<STOP; i++){
   ...
   if(bCalibrate){
     ...
     CalibratedFrequencyDomain(event.window_size, Trace, Amp, Phase, header.DAQ_id, 30.0, bRefAnt);
     delete[] Trace;
   }
    else TraceFFT((int)event.window_size, event.trace, Amp, Phase, Tracefft);
