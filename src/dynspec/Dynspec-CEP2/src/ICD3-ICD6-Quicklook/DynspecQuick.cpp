...
///   	 
///   	 First the code wil stock Root group metadata. Root Metadata are red by a function named readRoot and contained in the class Reader_Root_Quick.h, the
///   	  Root group is generated and Root's matadata are stocked, then written by a function named writeRootMetadata and contained in the class Stock_Write_Root_Metadata_Quick.h
///   	 
...
///   	 readDynspec and contained in the class Reader_Dynspec_Quick.h, after, metadata are stocked, then written by a function named writeDynspecMetadata contained in
...
///   	 Stock_Write_Dynspec_Data_Quick.h
...
int main(int argc, char *argv[])
{
  ...
  // PARAMETERS Determination
      ...
      for (i=0;i<obsNofSAP;i++)  // Loop on SAP
	{
	  ...
	  string index_h1;
	  std::ostringstream oss_h;oss_h << nofPart;string index_h(oss_h.str());
	  if (nofPart<10){index_h1="_P00"+index_h;}
	  if (nofPart>=10 && nofPart<100){index_h1="_P0"+index_h;}
	  if (nofPart>=100 && nofPart<1000){index_h1="_P"+index_h;}
	  
	  
	  string pathFile(listOfFiles[m]);
	  string outputFile(outputDir+"QuickLook_"+obsName+index_i1+index_h1+".h5");
	  ...
	  Stock_Write_Root_Metadata_Quick *rootMetadata;
	  rootMetadata = new Stock_Write_Root_Metadata_Quick();
	  // generation of an object which will read ROOT METADATA
	  Reader_Root_Quick *rootObject;
	  rootObject = new Reader_Root_Quick();
	  ...
	  for (j=0;j<obsNofBeam;j++)
 	    { 		    
		    ...
		    Reader_Dynspec_Quick *dynspecObject;
		    dynspecObject= new Reader_Dynspec_Quick();
		    
		    //generation of an object which will contains and write DYNSPEC METADATA  
		    Stock_Write_Dynspec_Metadata_Quick *dynspecMetadata;
		    dynspecMetadata = new Stock_Write_Dynspec_Metadata_Quick();

		    // generation of an object which will contains and write DYNSPEC DATA  
		    Stock_Write_Dynspec_Data_Quick *dynspecData;
		    dynspecData = new Stock_Write_Dynspec_Data_Quick();
