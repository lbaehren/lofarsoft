...
using namespace std;
  ...
  void Stock_Write_Dynspec_Data::writeDynspecData(Group &dynspec_grp,string pathDir,string obsName,string pathFile,string outputFile,File &root_grp,int i,int j,int k,int l,int q,int obsNofStockes,vector<string> stokesComponent,float memoryRAM)  
  {
      ...
      int p(0),m(0),n(0);
      int sizeTimeLimit((1.277E8*memoryRAM)/(m_Nspectral*obsNofStockes));	// time step ~ 1Go RAM memory maximum !
	...
	for (l=0;l<obsNofStockes;l++)
	  {
		...
		{    
		  ...
		  STOCKES.getMatrix( pos, size, &DATA_2D[0] );
	      ...
	      else
		{
		  if (nofLastElements > 0)
		    {
		      DATA_2D.resize(nofLastElements);
	  ...
	  if (p!=fracTime-1)
	    {	  
	      ...
	      data_grp.setMatrix( data_grp_pos, data_grp_size, &DATA_3D[0] );
	    }
	  else
	    {		  
	      if (nofLastElements > 0)
      ...
      cout << "Dynspec N°" << index_k << "  Ok" << endl;    
  }