...
int main(int argc, char *argv[])
{
  ...
  if (argc< 4 ){cout << "missing parameter: Dynspec  Observation-Path-DIR  Observation-Number(ex:Lxxxxx) Output-hdf5-File [Facultative:Dataset-RAM-allocation (in Go; default value: 1Go)] " << endl;}
  if (argc> 5 ){cout << "too much parameters: Dynspec  Observation-Path-DIR  Observation-Number(ex:Lxxxxx) Output-hdf5-File [Facultative:Dataset-RAM-allocation (in Go; default value: 1Go)] " << endl;}
  ...
  int obsNofSAP(file.nofSubArrayPointings().get());
  int obsNofBeam(SAP.nofBeams().get());  
  ...
  int methodFlagObsNofFrequencyBand(0);
  if (obsNofTiledDynspec == 0)
      ...
      while (res ==0)
	{
	  ...
	  methodFlagObsNofFrequencyBand = i-1;
	}
...
/*    
  ...
  string pathFile(pathDir+obsName+"_SAP000_B000_S0_P000_bf.h5");  
  ...
  rootObject->readRoot(pathFile,obsName,rootMetadata,obsNofSource,obsNofBeam,obsNofFrequencyBand,obsNofTiledDynspec);//,obsNofSource,obsNofBeam,obsNofStockes,obsNofFrequencyBand,obsNofTiledDynspec,obsMinFrequency,obsMaxFrequency,obsCentralFrequency);
  delete rootObject;  
	  ...
	  for (j=0;j<obsNofBeam;j++)
	    {
		    ...
		    dynspecObject->readDynspec(pathFile,dynspecMetadata,dynspecData,i,j,q,obsNofSAP,obsNofSource,obsNofFrequencyBand,obsNofStockes,stokesComponent);
		    ...
		    //WRITE DATA
		    dynspecData->writeDynspecData(dynspec_grp,pathDir,obsName,pathFile,outputFile,root_grp,i,j,k,l,q,obsNofStockes,stokesComponent,memoryRAM);
		    delete dynspecData;
