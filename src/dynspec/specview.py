...
for filename_h5 in filenames:         
    ...
    if (ds<ds_fits):
        ...
        try:
            Fits_data = pyfits.open(filename_fits)[0].data
        except IOError, err:
            os.system("python "+os.path.dirname(sys.argv[0])+"/downsample.py "
                      +filename_h5)
        else:
            print "reading "+filename_fits
        ds=int(ds/ds_fits)
    ...
    try:        Spectrum
        ...
        pylab.imshow(Spectrum,extent=(0,dt,fmax,fmin), aspect='auto',
