...
{  
  ...
  /* Check if we have already computed the polynomial coefficients up to the
     ...
     internal parameters. */
  if (l > order_p) {
    cerr << "[Hermite1D::fx] Adjusting internal coefficient cache "
	 << order_p << " -> " << l << " ..." << endl;
				   ...
				   std::vector<double> const &x)
{
  ...
  if (l.size() != x.size()) {
    std::cout << "[Hermite1D::fx] Mismatching array sizes!" << endl;
    y.resize(1,0.0);
  } else {
    nelem = x.size();
    y.resize(nelem,0.0);
				     ...
				     std::vector<double> const &x)
{
  ...
  // check if the vector sizes match
  if (l.size() != x.size()) {
    std::cout << "[Hermite1D::dfdx] Mismatching array sizes!" << endl;
    y.resize(1,0.0);
  } else {
    nelem = x.size();
    y.resize(nelem,0.0);
				       ...
				       std::vector<double> const &x)
{
  ...
  // check if the vector sizes match
  if (l.size() != x.size()) {
    std::cout << "[Hermite1D::d2fdx2] Mismatching array sizes!" << endl;
    y.resize(1,0.0);
  } else {
    nelem = x.size();
    y.resize(nelem,0.0);
