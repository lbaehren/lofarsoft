...
namespace LOPES {  // Namespace LOPES -- BEGIN
  ...
  bool CalTableWriter::GetPreSucRows (int const AntID,
	...
	*preRow = -1;
	...
	tmp = ROScalarColumn<unsigned int>(colTab,"StartDate")(*succRow);
  ...
  bool CalTableWriter::AddData (String data,
	...
	if ((preRowNr !=-1) && ROScalarColumn<unsigned int>(subtab,"StartDate")(preRowNr) == startDate) {
	...
	// fill the data
	ScalarColumn<unsigned int>(subtab,"StartDate").put(newRowNr,startDate);
	ScalarColumn<unsigned int>(subtab,"StopDate").put(newRowNr,stopDa);
	...
	if (preRowNr != -1) { ScalarColumn<unsigned int>(subtab,"StopDate").put(preRowNr,startDate);};
      };
      subtab.flush();
    } catch (AipsError x) {
  ...
  bool CalTableWriter::AddData (Array<double> data,
				int const AntID,
				String const FieldName, 
				unsigned int const startDate,
				unsigned int const stopDate) {
  ...
  bool CalTableWriter::AddData (Array<DComplex> data,
				int const AntID,
				String const FieldName, 
				unsigned int const startDate,
				unsigned int const stopDate) {
	...
	// set the StopDate of the preceding row if necessary
	if (preRowNr != -1) {
	  ScalarColumn<unsigned int>(subtab,"StopDate").put(preRowNr,startDate);
	}
      };
      subtab.flush();
    } catch (AipsError x) {
...
// ==============================================================================
  ...
  bool CalTableWriter::AddField (const String FieldName,
				 const String FieldDesc,
				 const String DataType, 
				 const bool shapeFixed,
				 const IPosition shape,
				 const bool SingleTables,
				 const String FriendField) {
    try {
      if (masterTable_p->actualTableDesc().isColumn(FieldName)){
	cerr << "CalTableReader::AddField "
	...
	cerr << "CalTableReader::AddField "
	     << "Cannot add a field to a table without antennas."
	     << endl;
	cerr << "CalTableReader::AddField "
	...
	if (shapeFixed){
	  if (shape.nelements() == 0) {
	    cerr << "CalTableReader::AddField: "
		 << "Array of fixed shape of length 0."
		 << endl;
	    return false;
	  };
	...
	return false;     
      ...
      entryDesc.addColumn(ScalarColumnDesc<unsigned int>("StartDate", "First date when this entry is valid"));
      entryDesc.addColumn(ScalarColumnDesc<unsigned int>("StopDate", "Last date when this entry is valid"));
  ...
  bool CalTableWriter::AddAntenna(int AntID,
    ...
    try {
      ...
      **indexedAnt_p = AntID;
      ...
      if (found) {
	cerr << "CalTableWriter::AddAntenna: "
	     << "Antenna ID already exists!"
	     << endl;
	return false;
      };
      // Add the row
	...
	cerr << "CalTableWriter::AddAntenna: "
	     << "The first two columns are not AntID and AntName!"
	     << endl;
	cerr << "The access to rows with AIPS++ tables sucks!"
	     << endl;
	return false;
      };
  ...
  bool CalTableWriter::SetKeyword (String data,
  ...
  bool CalTableWriter::SetKeyword (double data,
				   String const KeywordName) {
    try {
      if (masterTable_p->keywordSet().isDefined(KeywordName)) {
	masterTable_p->rwKeywordSet().removeField(KeywordName); //not nice but should work...
  ...
  bool CalTableWriter::SetKeyword (DComplex data,
				   String const KeywordName) {
    try {
      if (masterTable_p->keywordSet().isDefined(KeywordName)) {
	masterTable_p->rwKeywordSet().removeField(KeywordName); //not nice but should work...
  ...
  bool CalTableWriter::SetKeyword (Array<double> data,
				   String const KeywordName) {
    try {
      if (masterTable_p->keywordSet().isDefined(KeywordName)) {
	masterTable_p->rwKeywordSet().removeField(KeywordName); //not nice but should work...
  ...
  bool CalTableWriter::SetKeyword (Array<DComplex> data,
				   String const KeywordName) {
    try {
      if (masterTable_p->keywordSet().isDefined(KeywordName)) {
	masterTable_p->rwKeywordSet().removeField(KeywordName); //not nice but should work...
