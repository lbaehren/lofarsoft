...
/* $Id: DataReader.cc,v 1.29 2006/08/24 13:55:12 bahren Exp $*/
		       ...
		       Float const &samplerate)
{
  ...
  Vector<Float> adc2voltage (1,1.0);

  Matrix<Complex> fft2calfft (fftLength_p,1,1.0);
...
{
  Vector <Float> inColumn (blocksize_p,1.0);
  Vector<Complex> outColumn;
  FFTServer<Float,Complex> server(IPosition(1,blocksize_p),
...
Float DataReader::samplingRate (String const &unit)
{
...
void DataReader::setSamplingRate (Float const &samplingRate)
{
...
Vector<Double> DataReader::frequencyValues (Bool const &onlySelected)
{
  ...
  Float freqMin (samplingRate_p/2*(nyquistZone_p-1));
  Float freqResolution (samplingRate_p/(2*(fftLength_p-1)));
  uint nofChannels;
...
Matrix<Float> DataReader::fx ()
{
  Matrix<Float> out (blocksize_p,nofSelectedAntennas());
...
Matrix<Float> DataReader::voltage ()
{
  Matrix<Float> in (fx());
  Matrix<Float> out (in);
...
Matrix<Complex> DataReader::fft ()
{
  Matrix<Float> in (voltage());
  Matrix<Complex> out (fftLength_p,
			nofSelectedAntennas());
  
  Vector <Float> inColumn;
  Vector<Complex> outColumn;
  FFTServer<Float,Complex> server(IPosition(1,blocksize_p),
				    FFTEnums::REALTOCOMPLEX);
...
Matrix<Complex> DataReader::calfft ()
{
  int antenna(0);
  int channel(0);
  int nofSelectedAntennas (DataReader::nofSelectedAntennas());
  int nofSelectedChannels (DataReader::nofSelectedChannels());
  Matrix<Complex> in (fft());
  Matrix<Complex> out (nofSelectedChannels,nofSelectedAntennas);
...
Cube<Complex> DataReader::ccSpectra (Bool const &fromCalFFT)
{
  int antenna1 (0);
  int antenna2 (0);
  int channel  (0);
  Matrix<Complex> in (DataReader::nofSelectedChannels(),
  ...
  Cube<Complex> out (shape(0),shape(1),shape(1),0.0);
...
Matrix<Complex> DataReader::visibilities (Bool const &fromCalFFT)
{
  int antenna1 (0);
  int antenna2 (0);
  int channel  (0);
  ...
  IPosition shape (in.shape());
  Matrix<Complex> vis (shape(0),nofBaselines(false));
...
void DataReader::setADC2voltage (Vector<Float> const &adc2voltage)
{
  uint nelem (adc2voltage.nelements());

  if (nelem == nofSelectedAntennas()) {
    Vector<Float> tmp (nofAntennas());
    tmp = 1.0;
...
void DataReader::setFFT2calFFT (Matrix<Complex> const &fft2calfft)
{
  bool status (true);
...
void DataReader::setHanningFilter (Float const &alpha)
{
  if (alpha == 0.0) {
    applyHanning_p = False;
  } else {
    HanningFilter<Float> tmp (blocksize_p,alpha);
    hanningFilter_p = tmp;
    applyHanning_p = True;
  }
}

void DataReader::setHanningFilter (const Float &alpha,
				   const uint &beta)
{
  if (alpha == 0.0) {
    applyHanning_p = False;
  } else {
    HanningFilter<Float> tmp (blocksize_p,
			      alpha,
			      beta);
    hanningFilter_p = tmp;
    applyHanning_p = True;
...
Matrix<Complex> DataReader::selectChannels (Matrix<Complex> const &in)
{
  IPosition shape (in.shape());
  uint nofChannels (DataReader::nofSelectedChannels());
  Matrix<Complex> out (nofChannels,
		       nofSelectedAntennas());
