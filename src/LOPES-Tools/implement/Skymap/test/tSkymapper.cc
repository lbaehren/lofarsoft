...
/* $Id: tSkymapper.cc,v 1.10 2006/10/31 20:33:18 bahren Exp $*/
...
int test_PagedImage ()
{
  ...
  try {
    ...
    cout << " -- stepping through the distance axis ..." << endl;
    for (int radius(0); radius<shape(2); radius++) {
      start(2) = radius;
      pixels = Float(radius);
      //
      cout << radius << "\t" << start << "\t" << sum(pixels) << endl;
      //
...
/*!
  ...
  \param metafile  -- Metafile holding the information which data to use as 
                      input
  \param blocksize -- Number of samples per block of data.

  \return nofFailedTests -- The number of failed tests.
*/
Int test_Skymapper (string const &metafile,
		    uint const &blocksize)
{
  ...
  try {
    DataReader *dr;
    ITSCapture *capture = new ITSCapture (metafile,blocksize);
    dr = capture;

    ObservationData obsData ("LOPES");
    obsData.setObserver ("LOPES Project");
  ...
  try {
    ...
    // Coordinate settings
    IPosition shape (5,20,20,10,1,1);
...
//   {
//     nofFailedTests += test_Skymapper (metafile,
// 				      blocksize);
//   }
