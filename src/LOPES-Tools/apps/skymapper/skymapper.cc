...
/* $Id: skymapper.cc,v 1.10 2006/10/31 18:40:46 bahren Exp $ */
...
void signalStatistics (Matrix<DComplex>& antennaSignals)
{
  ...
  for (int i=0; i<nant; ++i) {
    ...
    sumAll += sumAntenna;
//     cout << "\tsum(" << i << "): " << sumAntenna;
//     cout << "\tmean: " << sumAntenna/nfreq << endl;
  }
...
Bool init_event (GlishSysEvent &event, void *) {
...

  {
    ...
    gtmp.get(elevation);
    //
    status = LOPES::QuantityFromRecord (epoch, "epoch", glishRec);
		    ...
		    void *)
{
  ...
  triggertime = LOPES::replaceString(LOPES::replaceString(LOPES::replaceString (triggertimes(0),"-","/"),"T","/"),"Z","");
...
Bool createImage (GlishSysEvent &event, void *)
{
  GlishSysEventSource *glishBus = event.glishSource();
  ...
  try {
    cout << " - extracting coordinate system from the GlishRecord ..." << endl;
    skymapper.setCoordinateSystem (event.val());
  } catch (AipsError x) {
    std::cerr << "[skymapper::createImage] " << x.getMesg() << std::endl;
  }

  cout << " - creating paged image from tiled shape ..." << endl;
  PagedImage<Double> image (shape,
			   skymapper.coordinateSystem(),
			   String("skymapper.img"));
  cout << " - image creation completed" << endl;

  if (glishBus->replyPending()) {
    glishBus->reply(GlishArray(True));
  } else {
    cerr << "[skymapper::createImage] No pending reply." << endl;
  }
...
{  
  ...
  glishStream.addTarget(createImage, "createImage");
