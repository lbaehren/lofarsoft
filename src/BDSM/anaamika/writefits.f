c! filen.img is the image to be written
        ...
        subroutine writefits(image,x,y,n,m,filen,fitsdir,fitsfiledir,
     /             scratch,fitsname)
        implicit none
...
c!  -----
     ...
     /      fitsname(1:nchar(fitsname))
         ...
         do i = 1,nkeys
          ...
          write (*,*) '|',record,'|',i,nkeys
          if (record.ne.'') then
           read (record,*) head
           if (head.ne.'SIMPLE'.and.head.ne.'BITPIX'.and.
     /        head(1:5).ne.'NAXIS'.and.head.ne.'EXTEND'.and.
     /        head.ne.'BSCALE')
