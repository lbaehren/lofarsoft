...
def write_fbdsm_gaul(img):
    ...
    hdr = img.header
    cdelt = N.array([abs(hdr['CDELT1']), abs(hdr['CDELT2']), abs(hdr['CDELT3'])])
    crpix = N.array([hdr['CRPIX1'], hdr['CRPIX2'], hdr['CRPIX3']])
    crval = N.array([hdr['CRVAL1'], hdr['CRVAL2'], hdr['CRVAL3']])
    ctype = N.array([hdr['CTYPE1'], hdr['CTYPE2'], hdr['CTYPE3']])
    for i in range(3):
        ctype[i] += ' '*(8-len(ctype[i]))
    bm_pix = N.array(img.pixel_beam)*fwsig
    try:
        crota = N.array([hdr['CROTA1'], hdr['CROTA2'], hdr['CROTA3']])
    except:
        crota = N.zeros(3)
