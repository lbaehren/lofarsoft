...
c! HARDCODE number of columns 31
     ...
     /       dumr,'i')
        close(22)
        call writesrlfits(f2,nsrc,31,srldir,fitsname,extname,fitsdir)
     ...
     /             extname,fitsdir)
        ...
        character*16 ttype(31),tform(31),tunit(31),code(nrows)*4
        ...
        data ttype/'src_id','flag','Code','Total_Flux',
     /    'Err_total_flux','Cen_Peak_flux','Err_Cpeak_flux',
     /    'Maximum_Peak_Flux','Err_Mpeak_flux',
     ...
     /    'isl_rms','num_gau'/
        data tform/'1J','1J','1A','1D','1D','1D','1D','1D','1D','1D',
     /    '1D','1D','1D','1D','1D','1D','1D','1D','1D','1D','1D','1D',
     /    '1D','1D','1D','1D','1D','1D','1D','1D','1J'/
...
c!
     ...
     /       '.srl.bin',form='unformatted')
        do i=1,nrows
         read (21) numsrc(i),sflag(i),code(i),total(i),etotfl(i),
...
c!
     ...
     /            extname,varidat,status)
        ...
        call ftpclj(unit,2,frow,felem,nrows,sflag,status)  
        call ftpcls(unit,3,frow,felem,nrows,code,status)  
        call ftpcld(unit,4,frow,felem,nrows,total,status)  
        call ftpcld(unit,5,frow,felem,nrows,etotfl,status)  
        call ftpcld(unit,6,frow,felem,nrows,speak,status)  
        call ftpcld(unit,7,frow,felem,nrows,epeakfl,status)  
        call ftpcld(unit,8,frow,felem,nrows,max_peak,status)  
        call ftpcld(unit,9,frow,felem,nrows,epeakfl,status)  
        call ftpcld(unit,10,frow,felem,nrows,sra,status) 
        call ftpcld(unit,11,frow,felem,nrows,era,status)
        call ftpcld(unit,12,frow,felem,nrows,sdec,status)
        call ftpcld(unit,13,frow,felem,nrows,edec,status)
        call ftpcld(unit,14,frow,felem,nrows,mra,status) 
        call ftpcld(unit,15,frow,felem,nrows,era,status)
        call ftpcld(unit,16,frow,felem,nrows,mdec,status)
        call ftpcld(unit,17,frow,felem,nrows,edec,status)
        call ftpcld(unit,18,frow,felem,nrows,bmaj,status)
        call ftpcld(unit,19,frow,felem,nrows,ebmaj,status)
        call ftpcld(unit,20,frow,felem,nrows,bmin,status)
        call ftpcld(unit,21,frow,felem,nrows,ebmin,status)
        call ftpcld(unit,22,frow,felem,nrows,bpa,status)
        call ftpcld(unit,23,frow,felem,nrows,ebpa,status)
        call ftpcld(unit,24,frow,felem,nrows,dbmaj,status)
        call ftpcld(unit,25,frow,felem,nrows,edbmaj,status)
        call ftpcld(unit,26,frow,felem,nrows,dbmin,status)
        call ftpcld(unit,27,frow,felem,nrows,edbmin,status)
        call ftpcld(unit,28,frow,felem,nrows,dbpa,status)
        call ftpcld(unit,29,frow,felem,nrows,edbpa,status)
        call ftpcld(unit,30,frow,felem,nrows,rmsisl,status)
        call ftpclj(unit,31,frow,felem,nrows,ngau,status)
