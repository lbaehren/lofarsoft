...
c! CHANGE GAUL FORMAT
        ...
        implicit none
        integer n,m,nchar,nsrc,nffmt,nisl,gpi
        real*8 keyvalue
        character f1*500,f2*500,ffmt*500
        character extn*10,keyword*500,fg*500,scratch*500
     ...
     /       srldir)
        ...
        integer xic(nisl),yic(nisl),nc,iisl,flag(nsrc)
        character f1*500,fn*500,extn*10,f2*500,ffmt*500,str1*1,fg*500
...
c         call pgsci(2)
c         call pgpoint(nsrc,x4,y4,2) 
c         call pgsci(1)
         do i=1,nsrc  ! plot fwhm ellipse
          if (flag(i).eq.0) then
c           call pgdrawellipse(x4(i),y4(i),bmaj4(i)/abs(cdelt(1)),
...
c         call pgsci(3)
         do i=1,n
          do j=1,m
c           if (mask(i,j).eq.1) call pgpoint(1,i*1.0,j*1.0,2)
          end do
         end do
c         call pgsci(1)
...
c         call pgsci(0)
         do iisl=1,nisl
c          call pgnumb(iisl,0,1,str3,nc)
c          call pgtext(xic(iisl)*1.0-0.5,yic(iisl)*1.0-0.5,str3)
         end do
c         call pgsci(1)
        end if
...
100     continue
        ...
        implicit none
        character f2*500,fn*500,extn*10,str*10
