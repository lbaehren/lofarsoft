...
c! do primary beam for src image, add to noise, write to fits.
        ...
        subroutine sim_addnoisrc(fn,imsize,nchan,freq,synbeam,crval1,
     ...
     /       50.d0,nptx*npty,xptcen,yptcen,maxpt,scratch)
        ...
        do 100 ifac=1,nmosx*nmosy     ! for each facet to cut out
         ...
         call sub_sim_addsrcnoi(fn,imsize,nchan,blc,trc,ptsizex,
...
100     continue
        ...
        subroutine sub_sim_addsrcnoi(fn,imsize,nchan,blc,trc,ptsizex,
     ...
     /             crval2,crpix1a,crpix2a,bw,fitsdir)
        ...
        real*8 sub_src(ptsizex,ptsizey,nchan)
        ...
        call subim3D(f1,imsize,imsize,nchan,sub_src,ptsizex,
     /               ptsizey,nchan,blc,trc)

        f1=fn(1:nchar(fn))//'.noi'
        call subim3D(f1,imsize,imsize,nchan,sub_noi,ptsizex,
     /               ptsizey,nchan,blc,trc)

c! forms final image of pointing ipt. 
        do i=1,ptsizex
         do j=1,ptsizey
          do k=1,nchan
           sub_im(i,j,k)=sub_src(i,j,k)+sub_noi(i,j,k)
          end do
         end do
        end do
