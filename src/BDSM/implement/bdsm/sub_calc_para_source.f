...
c! first get maxmem and nisl. 
     ...
     /    n,m,ctype,crpix,cdelt,crval,crota,bmar_p,bm_pix)
...
c! calc blc, trc and subimage size and pass on.
     ...
     /    delx,dely,xpixs,ypixs,bmar_p,bm_pix)
...
c! calculate the mask so each pixel is identified with a gaussian.
     ...
     /    delx,dely,xpixs,ypixs,bmar_p,bm_pix)
        ...
        real*8 maxva,max_peak,mpx,mpy,mra,mdec
...
c! makes mask. value at pixel is SOURCE number to which it belongs (see islnum)
     ...
     /      npix_isl,cdelt,delx,dely,islct,islnum,subim)
        ...
        do i=1,islct    ! for each source in the island
         ...
         if (ntimes.eq.1) then
          code='C  '
          numsrc=numsrc+1
          write (n1,777) numsrc,flag(h),code,totfl(h),etotfl(h),
     ...
     /     edbmaj(h),dbmin(h),edbmin(h),dbpa(h),edbpa(h),rmsisl(h),1
          write (n2) numsrc,flag(h),code,totfl(h),etotfl(h),
     /     peakfl(h),epeakfl(h),peakfl(h),epeakfl(h),ra(h),era(h),
     /     dec(h),edec(h),ra(h),era(h),dec(h),edec(h),bmaj(h),
     /     ebmaj(h),bmin(h),ebmin(h),bpa(h),ebpa(h),dbmaj(h),
     /     edbmaj(h),dbmin(h),edbmin(h),dbpa(h),edbpa(h),rmsisl(h),1
     ...
     /     mpy,mra,mdec)
          ...
          write (n1,777) numsrc,sflag,code,total,etotfl(1),
     /     speak,epeakfl(1),max_peak,epeakfl(1),sra,era(1),
     /     sdec,edec(1),mra,era(1),mdec,edec(1),
     /     mompara(4)*cdelt(1)*3600.d0,ebmaj(1),
     /     mompara(5)*cdelt(2)*3600.d0,ebmin(1),
     /     mompara(6)+90.d0,ebpa(1),gaus_d(1)*fwsig*cdelt(1)*3600.d0,
     /     edbmaj(1),gaus_d(2)*fwsig*cdelt(2)*3600.d0,edbmin(1),
     /     gaus_d(3)+90.d0,edbpa(1),rmsisl(1),ngau
          write (n2) numsrc,sflag,code,total,etotfl(1),
     /     speak,epeakfl(1),max_peak,epeakfl(1),sra,era(1),
     /     sdec,edec(1),mra,era(1),mdec,edec(1),
     /     mompara(4)*cdelt(1)*3600.d0,ebmaj(1),
     /     mompara(5)*cdelt(2)*3600.d0,ebmin(1),
