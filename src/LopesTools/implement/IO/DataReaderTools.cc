...
/* $Id: DataReaderTools.cc,v 1.4 2006/08/09 10:05:06 bahren Exp $*/
					       ...
					       bool const &verbose)
{
  ...
  Matrix<Complex> fft2calfft (fft2calfft (rec));
  ...
  if (nofMissingInputs > 0) {
    ...
    dr->setSamplingRate (samplingRate);
...
bool DataReaderTools::getSamplingRate (Float &samplingRate,
				       GlishRecord &rec)
{
  GlishArray gtmp;
  bool status (true);
...
bool DataReaderTools::getAdc2voltage (Vector<Float> &adc2voltage,
				      GlishRecord &rec)
{
  GlishArray gtmp;
  bool status (true);
...
Matrix<Complex> DataReaderTools::fft2calfft (GlishRecord &rec)
{
  GlishArray gtmp;
