...
void DataCC::setDatafile () 
{
  ...
  if (datafiles(0,0) != "") {
    datafileName = StringTools::fileFromPath (datafiles(0,0));
  }
...
void DataCC::setFrequencyChannels ()
{
  ...
  Vector<String> sub1 = StringTools::getSubstrings (plugin1," ");
  Vector<String> sub2 = StringTools::getSubstrings (plugin2," ");
  IPosition ip;
  ...
  for (int i=0; i<ip(0); i++) {
    ...
    if (sub1(i) == "-e") {
      ...
      nofIntervals = StringTools::nofSubstrings (sub1(i+1),",");
      Vector<String> intervals = StringTools::getSubstrings (sub1(i+1),",");
      frequencyChannels.resize(nofIntervals,2);
      frequencyChannels = -1;
      // decompose the range
      for (int j=0; j<nofIntervals; j++) {
	Vector<String> tmp = StringTools::getSubstrings (intervals(j),":");
