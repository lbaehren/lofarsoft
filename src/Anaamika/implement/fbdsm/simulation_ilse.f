...
c! modify for ilse so u dont convolve ... 
...
c! now obtain flux, x, y, populate into image, prepare source list 
        call cr8sky_ilse(freq,bw,synbeam,cdelt1,imsize,bmsampl,
     /       min(50*int(bmsampl),imsize),nchan,sens,cn1,sarr,
     /       gam1,100,seed,fn,std,scratch,beamvary,bmvaryf,bmvaryn)

cc! create noise map seperately, convolve with gaussian using FFT. add src+noi later
c        call sim_cr8noise(fn,sens,nchan,imsize,bmsampl,seed)
...
c! now de-mosaic the image and add src and noise to create subcubes, write fits directly
        call sim_addnoisrc_ilse(fn,imsize,nchan,freq,synbeam,
     /   crval1,crval2,crpix1,crpix2,cdelt1,cdelt2,bw,scratch,fitsdir)
