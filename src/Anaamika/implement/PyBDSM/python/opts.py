...
class Opts(object):
    ...
    use_pyrap      = Option(None, Bool(), doc="If None, Pyfits is tried first and then Pyrap. If set to bool, \
                    will crash if True and not available")
    stop_at        = Option(None, String(), doc="Stops after step if 'isl' or 'fit' or 'resid'")
    ...
    spline_rank    = Enum(3,1, doc="rank of the interpolating function " \
                              "for rms/mean map")
    ...
    minpix_isl     = Int(4, doc="minimal number of pixels with " \
                             "emission per island")
    ini_gausfit    = String('fbdsm', doc = "Initial guess for gaussians, 'default', 'fbdsm', or \
                     'nobeam'. See manual for details")

    flag_smallsrc   = Bool(True, doc = "Flag sources smaller than the flag_minarea times beam area")
    ...
    solnname       = Option(None, String(), doc="Name of the run, to be appended to the name of the output directory")
    indir          = Option(None, String(), doc="Directory of input FITS files")
