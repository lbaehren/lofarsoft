c! write gsm_table_solnname and gsm_calcf_solnname and gsm_posn_solnname
     ...
     /             ra,dec,era,edec,avra,eavra,avdec,eavdec)
        ...
        real*8 dtot(nf,maxngaul),espin_arr(sumngaul,np),totfl,etotfl
...
c! write headers in gsm_calcf_solnname
     ...
     /       str3(nchar(str3):nchar(str3))=' '
         str1=str1(1:nchar(str1))//'      S_'//
     /        str3(bnchar(str3):nchar(str3))//'       Err'
         str2=str2(1:nchar(str2))//'        Jy          Jy'
        end do
        do i=1,b_order
        ...
        do i=1,nassoc
         ...
         do inf=1,g_order
          if (assoc_arr(i,f_ord_g(inf)).eq.0) then
           ...
           etotfl=dtot(f_ord_g(inf),assoc_arr(i,f_ord_g(inf)))
          end if
          write (n2,'(1Pe10.3,a1,1Pe10.3,a1,$)') totfl,' ',etotfl,' '
         end do
         do inf=1,b_order
          if (assoc_arr(i,f_ord_b(inf)).eq.0) then
           ...
           etotfl=dtot(f_ord_b(inf),assoc_arr(i,f_ord_b(inf)))
           calcfl=calcflux0(f_ord_b(inf),assoc_arr(i,f_ord_b(inf)))
          end if
          write (n2,'(3(1Pe10.3,a1),$)') totfl,' ',etotfl,' ',calcfl,' '
         end do
         write (n2,*)
        end do
