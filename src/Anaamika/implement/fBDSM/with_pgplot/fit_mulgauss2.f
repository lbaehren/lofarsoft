...
c! start from nmulsrc=1 to max=nmulsrc
     ...
     /      fitfreely,iniguess,flagsmallsrc)
        ...
        real*8 e_x0_x(max_msrc),e_y0_y(max_msrc)
...
c! error analysis. do basic stuff from condon for now.
        do i1=1,nmulsrc
         ...
         e_x0_x(i1)=a(4+d)*sq2/(d1*dumrr3*(dumrr1**1.25d0)*
     /            (dumrr2**0.25d0))
         e_y0_y(i1)=a(5+d)*sq2/(d1*dumrr3*(dumrr1**0.25d0)*
     /            (dumrr2**1.25d0))
     ...
     /      e_min(i1)*e_min(i1)/(a(5+d)*a(5+d))))
         ...
         e_x0(i1)=sqrt(e_x0_x(i1)*e_x0_x(i1)*dsin(bpa)*dsin(bpa)+
     /                 e_y0_y(i1)*e_y0_y(i1)*dcos(bpa)*dcos(bpa))
         e_y0(i1)=sqrt(e_x0_x(i1)*e_x0_x(i1)*dcos(bpa)*dcos(bpa)+
     /                 e_y0_y(i1)*e_y0_y(i1)*dsin(bpa)*dsin(bpa))
        end do
