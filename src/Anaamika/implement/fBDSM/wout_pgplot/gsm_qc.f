...
c! plot the median spins
     ...
     /     gsmsolnname(1:nchar(gsmsolnname))
        ...
        do i=1,2
ccc         if (i.eq.1) call pgbegin(0,f1,1,1)
ccc         if (i.eq.2) call pgbegin(0,f2,1,1)
         if (i.eq.1) write (21,*) 
...
ccc         call pgwindow(mnx-0.1*(mxx-mnx),mxx+0.1*(mxx-mnx),mny,mxy)
ccc         call pgbox('BCNLST',0.0,0,'BCNLST',0.0,0)
ccc         if (i.eq.1) call pglabel('Frequency (MHz)','Peak flux ',' ')
ccc         if (i.eq.2) call pglabel('Frequency (MHz)','Total flux ',' ')
         dumi=1
         do j=1,nf
...
ccc           call pgsci(dumi)
ccc           call pgpoint(2,x,y,15)
ccc           call pgline(2,x,y)
           dumi=dumi+1
