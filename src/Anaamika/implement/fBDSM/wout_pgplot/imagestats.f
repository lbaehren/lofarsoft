...
ccc        call pghist(dumi,arr4,mn1,mx1,100,0)
ccc        call pgqwin(mn1,mx1,mn2,mx2)
ccc        call pgend

        strdev='/xs'
ccc        id1=pgopen(strdev(1:nchar(strdev)))   ! image
        if (id1.le.0) stop
ccc        call pgslct(id1)
ccc        call pgpap(9.0,1.0)
ccc        call pgpage
ccc        call pgask(.false.)
ccc        call pgvport(0.1,0.9,0.3,0.9)
...
c        end if
        ...
        mx=av4+10.0*std4
ccc        call pgsci(3)
ccc        call pgwindow(0.5,n+0.5,0.5,m+0.5)
ccc        call pgbox('BCINST',0.0,0,'BCINST',0.0,0)
ccc        call pggray(arr4,n,m,1,n,1,m,mx,mn,tr)
ccc        call pgwedg('RG',2.0,3.0,mx,mn,' ')
ccc        call pgsci(1)
...
ccc        call pgtext(0.3,0.97,'Rect. area    or irregular area    or '//
...
ccc        call pgwindow(0.5,n+0.5,0.5,m+0.5)
ccc        call pgcurs(xcur,ycur,ch1)
ccc        call pgsci(6)
ccc        call pgband(2,1,xcur,ycur,xcur1,ycur1,ch1)
        call setzoom(xcur,ycur,xcur1,ycur1,blc,trc,n,m)
ccc        call pgsfs(2)
ccc        call pgrect(blc(1),trc(1),blc(2),trc(2))
ccc        call pgsfs(1)
ccc        call pgsci(1)
...
ccc        call pgvport(0.1,0.9,0.1,0.9)
ccc        call pgwindow(0.5,n+0.5,0.5,m+0.5)
ccc        call pgbox('BCNST',0.0,0,'BCNST',0.0,0)
...
ccc        call pgsfs(1)
        if (xx.ge.1.and.xx.le.n.and.yy.ge.1.and.yy.le.m) then
         ...
         xver(nver)=xx
         yver(nver)=yy
ccc         call pgrect(xx-0.5,xx+0.5,yy-0.5,yy+0.5)
         if (nver.gt.1) then
ccc          call pgmove(xver(nver-1)*1.0,yver(nver-1)*1.0)
ccc          call pgdraw(x,y)
         end if
         goto 333
        end if
ccc        call pgmove(xver(1)*1.0,yver(1)*1.0)
ccc        call pgdraw(xver(nver)*1.0,yver(nver)*1.0)
ccc        call pgsci(1)
...
ccc        call pgsci(6)
ccc        call pgrect(xuse(i,1)-0.5,xuse(i,1)+0.5,yuse(i,1)-0.5,
 
ccc        call pgsci(7)
ccc        call pgrect(xuse(i,2)-0.5,xuse(i,2)+0.5,yuse(i,2)-0.5,
...
ccc           call pgsfs(1)
ccc           call pgsci(4)
ccc           call pgrect(i-0.5,i+0.5,j-0.5,j+0.5)
          end if
