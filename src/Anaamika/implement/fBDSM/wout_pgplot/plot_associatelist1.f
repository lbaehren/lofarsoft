...
ccc         id=pgopen(fname(1:nchar(fname)))
...
ccc        call pgsch(1.1)
ccc        call pgslw(3)
ccc        call pgvport(0.66,1.0,0.35,0.9)
ccc        call pgsch(0.6)
ccc        call pgtext(0.1,0.8,scrat(1:nchar(scrat)))
ccc        call pgsch(1.3)
        call get_pgnumb(avx,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.7,'Av X='//str1(1:nc))
        call get_pgnumb(avy,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.6,'Av Y='//str1(1:nc))
        call get_pgnumb(stdx,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.5,'Std X='//str1(1:nc))
        call get_pgnumb(stdy,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.4,'Std Y='//str1(1:nc))
        call get_pgnumb(medx,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.3,'Med X='//str1(1:nc))
        call get_pgnumb(medy,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.1,0.2,'Med Y='//str1(1:nc))
ccc        call pgiden

ccc        call pgvport(0.1,0.65,0.4,0.95)
...
ccc        call pglabel(label1(1:nchar(label1)),
        ...
        if (clip.eq.'n') then
ccc         call pgpoint(n,xplot,yplot,dumi)
        else
         do i=1,n
ccc          if (mask(i).eq.1) call pgpoint(1,xplot(i),yplot(i),dumi)
         end do
        end if

        if (label1(1:6).eq.label2(1:6)) then
ccc         call pgmove(mnx,mny)
ccc         call pgdraw(mxx,mxy)
        end if
        if (mark) then
         do 200 i=1,n
          if (xplot(i).ge.mnx.and.xplot(i).le.mxx.and.
     /        yplot(i).ge.mny.and.yplot(i).le.mxy) then
ccc           call pgnumb(i,0,1,chrid,nc)
ccc           call pgtext(xplot(i),yplot(i),chrid)
          end if
200      continue
        end if

ccc        call pgvport(0.05,0.44,0.05,0.25)
        if (label1(1:6).eq.label2(1:6)) then
ccc         call pgwindow(minxo,maxxo,min(mnx1,mny1),max(mxx1,mxy1))
        else
ccc         call pgwindow(minxo,maxxo,mnx1,mxx1)
        end if
ccc        call pgbox('BCNST',0.0,0,'BCVNST',0.0,0)
ccc        call pghist(n,xplot,minxo,maxxo,100,1)
        dum41=minxo+0.1*(maxxo-minxo)
        dum42=mxx1-0.2*(mxx1-mnx1)
ccc        call pgtext(dum41,dum42,label1(1:nchar(label1)))

ccc        call pgvport(0.56,0.95,0.05,0.25)
        if (label1(1:6).eq.label2(1:6)) then
ccc         call pgwindow(minxo,maxxo,min(mnx1,mny1),max(mxx1,mxy1))
        else
ccc         call pgwindow(minyo,maxyo,mny1,mxy1)
        end if
ccc        call pgbox('BCNST',0.0,0,'BCVNST',0.0,0)
ccc        call pghist(n,yplot,minyo,maxyo,100,1)
        dum41=minyo+0.1*(maxyo-minyo)
        dum42=mxy1-0.2*(mxy1-mny1)
ccc        call pgtext(dum41,dum42,label2(1:nchar(label2)))
ccc        call pgclos
