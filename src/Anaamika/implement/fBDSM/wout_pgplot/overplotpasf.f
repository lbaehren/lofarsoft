...
c! ===============  circle associations or draw vectors =============
        ...
        if (choi.eq.'c') then
         ...
         do i=1,npairs
          ...
          sizep=20.0
ccc          call pgdrawellipse(0.5*(x41(i)+x42(i)),0.5*(y41(i)+y42(i)),
 
         end do
        end if
        if (choi.eq.'d') then
ccc         call pgsci(starc)
         do i=1,npairs
     ...
     /       (x41(i)-x42(i))*(x41(i)-x42(i)))
          ...
          yp2=slop*xp2+intr
ccc          call pgslw(3)
ccc          call pgmove(xp1,yp1)
ccc          call pgdraw(xp2,yp2)
ccc          call pgslw(1)
ccc          call pgsch(1.7)
          if (abs(xp1-x41(i)).lt.abs(xp1-x42(i))) then
ccc           call pgarroww(xp1,yp1,xp2,yp2,slop,stfac*l1)
          else
ccc           call pgarroww(xp2,yp2,xp1,yp1,slop,stfac*l1)
          end if
ccc          call pgsch(1.0)
         end do
        end if
...
ccc         call pgsch(1.2)
         do i=1,npairs
          xd1=var(1,i)/var(7,i)
ccc          call pgnumb(int(xd1*10),-1,0,str2,nc)
ccc          call pgtext(x41(i),y41(i),str2(1:nchar(str2)))
         end do
ccc         call pgsch(1.0)
ccc         call pgsci(1)
        end if
...
c           dmin=dmin*dcos(var(3,i)/rad)  ! why do i do this ?
           ...
           dpa=0.0!var(6,i)
ccc           call pgdrawellipse(x41(i),y41(i),dmaj,dmin,dpa,
 
          else
c           dmin=dmin*dcos(var(9,i)/rad)
           dmaj=dmin!*var(10,i)/var(11,i)
           dpa=0.0!var(12,i)
ccc           call pgdrawellipse(x42(i),y42(i),dmaj,dmin,dpa,
...
ccc           call pgtext(x41(i),y41(i),naam(1:nchar(naam)))
         end if
         close(21)
        end do
ccc        call pgsch(1.0)
...
ccc         call pgmove(xp,yp)
ccc         call pgdraw(xx,yy)
        end do
