...
ccc      call pgvport(0.1,0.9,0.4,0.9)
...
ccc      call pgwedg('RG',1.5,3.0,mx,mn,' ')

      call vec8to4(summ,rnum,sum4,rnum)
      call vec8to4(radi,rnum,radi4,rnum)
ccc      call pgvport(0.1,0.9,0.1,0.4)
      call rangevec8(summ,rnum,rnum,m1,m2)
      mny=m1
      mxy=m2
      mxx=radi(rnum)
ccc      call pgwindow(0.0,mxx,mny,mxy)
ccc      call pgbox('BCINST',0.0,0,'BICNST',0.0,0)
ccc      call pgline(rnum,radi4,sum4)      
ccc      call pgend
