...
ccc        id1=pgopen(str1(1:nchar(str1)))  ! menu
        if (id1.le.0) stop
ccc        call pgslct(id1)
ccc        call pgpap(4.5,1.0)
ccc        call pgpage
ccc        call pgask(.false.)
        blackm=0

ccc        id2=pgopen(str1(1:nchar(str1)))   ! image
        colour=.false.                    ! start with greyscale
        if (id2.le.0) stop
ccc        call pgslct(id2)
ccc        call pgpap(9.0,1.0)
ccc        call pgpage
ccc        call pgask(.false.)
        starc=1 
        cs='grey'
...
234     continue
ccc        call pgslct(id1)
ccc        call pgvport(0.0,1.0,0.0,1.0)
        call clickopn(16,nmaxopts,0.03,ch1,opts,blackm)
ccc        call pgslct(id2)

        if (ch1.eq.'1') then                    !! Zoom in
         vport(3)=dum4
ccc         call pgsub_op1(vport,blc,trc,i,j,ch1,chold)
         goto 444
        end if
        if (ch1.eq.'2') then                    !! Zoom out
ccc         call pgsub_op2(ch1,chold)
         goto 333
        end if
        if (ch1.eq.'3') then                    !! Greyscale
ccc         call pgsub_op3(colour,cs,ch1,chold)
         goto 333
        end if
        if (ch1.eq.'4') then                    !! Colour
ccc         call pgsub_op4(colour,cs,str1,blacki,ch1,chold)
         goto 333
        end if
...
ccc         call pgpage
         ...
         call sub_grey_drawimage(id2,hi,blc,trc,arr4,i,j,up,low,tr,lab,
     /    cod1,mn1,mx1,mn2,mx2,lowh,uph,dum4,cs,str1,blacki,logged)
ccc         call pgsub_op5(mn1,mx1,mn2,mx2,low,up,ch1,chold)
         hi=hiold
         goto 444
        end if
        if (ch1.eq.'6') then                    !! Imagescale - area
ccc         call pgsub_op6(dum4,blc,trc,i,j,arr4,low,up,ch1,chold)
         goto 333
        end if
        if (ch1.eq.'7') then                    !! Imagescale - pixrange
ccc         call pgsub_op7(low,up,ch1,chold,logged,low1)
         goto 444
        end if
        if (ch1.eq.'8') then                    !! Get pixel
ccc         call pgsub_opE(dum4,blc,trc,ch1,chold,array,x,y)
         goto 333
        end if
        if (ch1.eq.'9') then                    !! Toggle linear/log
ccc         call pgsub_opF(ch1,chold,logged,array,x,y,arr4,i,j,low,up,low1)
         goto 333
        end if
        if (ch1.eq.'A') then                    !! Plot gaussians
ccc         call pgsub_op8(srldir,dum4,blc,trc,ch1,chold,
 
         goto 234
        end if
        if (ch1.eq.'B') then                    !! Name stars
ccc         call pgsub_op9(srldir,dum4,blc,trc,blacki,scratch,ch1,chold)
         goto 234
        end if
        if (ch1.eq.'C') then                    !! Plot islands
ccc         call pgsub_opA(scratch,dum4,blc,trc,starc,linec,ch1,chold,
 
         goto 234
        end if
        if (ch1.eq.'D') then                    !! Plot coordinate grid
ccc         call pgsub_opB(f1,scratch,dum4,blc,trc,ch1,chold)
         goto 234
        end if
        if (ch1.eq.'E') then                    !! Plot associatelist
ccc         call pgsub_opC(f1,scratch,dum4,blc,trc,starc,id1,id2,str1,hi,
 
         goto 234
        end if
        if (ch1.eq.'F') then                    !! Plot gaussians
ccc         call pgsub_opD(srldir,dum4,blc,trc,ch1,chold,
...
c         up2=up2*1.1
        ...
        dumi=i*j
ccc        call pgbegin(0,'/NULL',1,1)
ccc        call pghist(dumi,arr4,low,up,100,0)
ccc        call pgqwin(mn1,mx1,mn2,mx2)
ccc        call pghist(dumi,arr42,low2,up2,100,0)
ccc        call pgqwin(mn12,mx12,mn22,mx22)
ccc        call pgend

ccc        call pgbegin(0,str1(1:nchar(str1)),1,1)
ccc        call pgvport(0.12,0.52,0.45,0.9)
ccc        call pgwindow(0.5,i*1.0+0.5,0.5,j*1.0+0.5)
ccc        call pgbox('BCINST',0.0,0,'BCINST',0.0,0)
ccc        call pggray(arr4,i,j,1,i,1,j,up,low,tr)
ccc        call pglabel(' ',' ',lab1)

ccc        call pgvport(0.58,0.98,0.45,0.9)
ccc        call pgwindow(0.5,i*1.0+0.5,0.5,j*1.0+0.5)
ccc        call pgbox('BCNIST',0.0,0,'BCNIST',0.0,0)
ccc        call pggray(arr42,i,j,1,i,1,j,up2*0.5,low2,tr)
ccc        call pglabel(' ',' ',lab2)

ccc        call pgvport(0.12,0.52,0.1,0.4)
ccc        call pgwindow(mn1,mx1,mn2,mx2)
ccc        call pgbox('BCNST',0.0,0,'BCVNST',0.0,0)
ccc        call pghist(i*j,arr4,low,up,100,1)
ccc        call pgwedg('BG',1.5,3.0,up,low,' ')

ccc        call pgvport(0.58,0.98,0.1,0.4)
ccc        call pgwindow(mn12,mx12,mn22,mx22)
ccc        call pgbox('BCNST',0.0,0,'BCVNST',0.0,0)
ccc        call pghist(i*j,arr42,low2,up2,100,1)
ccc        call pgwedg('BG',1.5,3.0,up2*0.5,low2,' ')
...
ccc        call pgbox('BCNST',0.0,0,'BNST',0.0,0)
ccc        call pgvport(0.1,0.9,0.1,0.9)
ccc        call pglabel(xl,yl1,' ')
        if (nc.ge.1) then
ccc         if (code(1:1).eq.'L') call pgline(nbin,x4,y41)
ccc         if (code(2:2).eq.'P') call pgpoint(nbin,x4,y41,7)
        end if
        if (nc.ge.2) then
         call vec8to4(y2,num,y42,nbin)
         call range2(y42,nbin,nbin,d41,d42)
ccc         call pgwindow(mnx,mxx,d41,d42)
ccc         call pgbox('',0.0,0,'CMST',0.0,0)
ccc         call pgptxt(mxx+0.08*(mxx-mnx),0.5*(d42+d41),90.0,0.5,yl2)
ccc         call pgsls(2)
ccc         if (code(3:3).eq.'L') call pgline(nbin,x4,y42)
ccc         if (code(4:4).eq.'P') call pgpoint(nbin,x4,y42,7)
        end if
        if (nc.eq.3) then
ccc         call pgsls(3)
ccc         if (code(5:5).eq.'L') call pgline(nbin,x4,y43)
ccc         if (code(6:6).eq.'P') call pgpoint(nbin,x4,y43,7)
        end if
ccc        call pgend
...
ccc        call pgsfs(2)
ccc        call pgcirc(x1,y1,rad)
ccc        call pgcirc(x2,y2,rad)
ccc        call pgsfs(1)
        x=x2
        y=y2
ccc        call pgcurs(x,y,ch)      
...
ccc        call pgpage
ccc        call pgwindow(0.0,1.0,0.0,1.0)
ccc        call pgsch(1.8)
ccc        call pgsci(blackm+8)
        do i=1,n
         xa(i)=0.1
...
ccc        call pgsfs(2)
        do i=1,n
ccc         call pgcirc(xa(i),ya(i),rad)
ccc         call pgtext(xa(i)+0.1,ya(i),opts(i)(1:nchar(opts(i))))
        end do
ccc        call pgsfs(1)
ccc        call pgsch(1.0)
...
ccc        call pgbin(nbin,mid,histo,.true.)

        do i=1,nbin-1
ccc         call pgmove(high(i),0.0)
ccc         call pgdraw(high(i),histo(i))
        end do
...
ccc        call pgsci(blacki+starc)
ccc        call pgslw(2)
cccc        call pgsls(linec)
ccc        call pgline(38,x2,y2)
ccc        call pgsci(blacki+1)
ccc        call pgslw(1)
ccc        call pgsls(1)
...
ccc          call pgdrawellipse(x4(i),y4(i),bmaj4(i)*1.0,bmin4(i)*1.0,
...
ccc        call pgbegin(0,'/NULL',1,1)
ccc        call pgsci(blacki)
ccc        call pghist(nsrc,plotx,minx,maxx,100,0)
ccc        call pgqwin(minx,maxx,mn1,mx1)
ccc        call pgsci(blacki+1)
ccc        call pgend
...
ccc        call pgvport(0.1,0.9,dum4,0.9)
ccc        call pgsci(blacki+1)
ccc        call pgwindow(blc(1)-0.5,trc(1)+0.5,blc(2)-0.5,trc(2)+0.5)
ccc        call pgbox('BCINST',0.0,0,'BICNST',0.0,0)

        if (colourscheme.eq.'grey') then
ccc         call pggray(arr4,i,j,1,i,1,j,up,low,tr)
ccc         call pgwedg('RG',1.5,3.0,up,low,' ')
        else
ccc         call pgimag(arr4,i,j,1,i,1,j,low,up,tr)
ccc         call pgwedg('RI',1.5,3.0,low,up,' ')
        end if
ccc        call pglabel(' ',' ',lab(1:nchar(lab)))
        call set_colourmap(colourscheme,clr1,clr2)    ! sets colourmap

        if (hi.eq.1) then
ccc         call pgvport(0.1,0.9,0.1,0.3)
         if (cod1.eq.'hn') then
ccc          call pgwindow(mn1,mx1,mn2,mx2)
ccc          call pgbox('BNST',0.0,0,'BCVNST',0.0,0)
ccc          call pghist(i*j,arr4,lowh,uph,100,1)
         end if
         if (cod1.eq.'hg') then
          mn2=0.0
          mx2=log10(mx2)
ccc          call pgwindow(mn1,mx1,mn2,mx2)
ccc          call pgbox('BNST',0.0,0,'BCVNSTL',0.0,0)
...
ccc        call pgsci(blacki+starc)
        do i=1,n
         do j=1,m
ccc          if (mask(i,j).eq.1) call pgpoint(1,i*1.0,j*1.0,20)
         end do
        end do

        call readisllist_getisln(srlname,nisl,xic,yic,scratch)
ccc        call pgsci(blacki+3)
        do iisl=1,nisl
ccc         call pgnumb(iisl,0,1,str6,nc)
ccc         call pgtext(xic(iisl)*1.0-0.5,yic(iisl)*1.0-0.5,str6)
        end do
ccc        call pgsci(blacki+1)
...
ccc        id3=pgopen(str1(1:nchar(str1)))  ! menu
        if (id3.le.0) stop
ccc        call pgslct(id3)
ccc        call pgpap(2.5,1.0)
ccc        call pgpage
ccc        call pgask(.false.)
345     continue  
ccc        call pgvport(0.0,1.0,0.0,1.0)
...
ccc           call pgtext(x4,y4,nam(1:nchar(nam)))
          end if
         end if
        end do
ccc        call pgsch(1.0)
...
ccc        call pgbegin(0,strdev,1,1)
        ...
        vport(3)=0.15
        vport(4)=0.85
ccc        call pgsub_op1(vport,blc,trc,n,m,ch1,ch2)
ccc        call pgsci(5)
ccc        call pgsfs(2)
ccc        call pgrect(blc(1)*1.0,trc(1)*1.0,blc(2)*1.0,trc(2)*1.0)
ccc        call pgsci(1)
ccc        call pgsfs(1)
