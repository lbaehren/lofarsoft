...
337     read (25,'(a)') head
        if (head(2:5).ne.'gaul') then
         write (29,'(a)') head(1:nchar(head))
         goto 337
        end if
        close(25)
        write (29,'(a)') 'src# isl# flag code tot_Jy err CenPkJy err '
     /   //'MaxPkJy err'
     ...
     /   //' &errors rms_isl num_gaus blc1 blc2 trc1 trc2 imrms'
        write (29,'(a)') 'fmt 85 '//'"(3(i7,1x),a4,1x,6(1Pe11.3,1x),'//
...
333     continue
     ...
     /    call process_g2s(29,23,24,gpi,cisl,islold,flag,totfl,etotfl,
...
334     continue
        if (cisl.gt.0) 
     /   call process_g2s(29,23,24,gpi,cisl,isl,flag,totfl,etotfl,
     /    peakfl,epeakfl,ra,era,dec,edec,xpix,expix,ypix,eypix,
     /    bmaj,ebmaj,bmin,ebmin,bpa,ebpa,dbmaj,edbmaj,dbmin,edbmin,
     /    dbpa,edbpa,rmssrc,avsrc,rmsisl,avisl,chisq,q,numsrc, 
     /    thresh_isl,rms_clip,ctype,crpix,cdelt,crval,crota,scratch,
