...
c! plot fluxes
ccc          id=pgopen(plotname(1:nchar(plotname)))
ccc          call pgslct(id)
ccc          call pgvport(0.1,0.95,0.13,0.95)
ccc          call pgslw(3)
ccc          call pgsch(1.4)
...
ccc          call pgbox('BCNLST',0.0,0,'BCNLST',0.0,0)
ccc          call pglabel('Freq (MHz)','Flux (Jy)',' ')
          do j=1,nfit
           if (ftofit(j).eq.1) then
ccc            call pgpoint(1,freq4(j),flux4(j),16)
           else
ccc            call pgpoint(1,freq4(j),flux4(j),22)
           end if
          end do
ccc          call pgerry(nfit,freq4,y1,y2,1.0)
...
ccc            call pgtext(mnx+0.05*(mxx-mnx),mny+0.05*(mxy-mny)+(j-1)*
...
ccc        call pgnumb(mm,pp,formm,str,nc)
ccc        call pgtext(x,y,str1(1:nchar(str1))//str(1:nc))
