...
ccc         call pgsci(2)
ccc         call pgpoint(nsrc,x4,y4,2) 
ccc         call pgsci(1)
         do i=1,nsrc  ! plot fwhm ellipse
          if (flag(i).eq.0) then
ccc           call pgdrawellipse(x4(i),y4(i),bmaj4(i)/abs(cdelt(1)),
...
ccc         call pgsci(3)
         do j=1,m
          do i=1,n
ccc           if (mask(i,j).eq.1) call pgpoint(1,i*1.0,j*1.0,2)
          end do
         end do
ccc         call pgsci(1)
...
ccc         call pgsci(0)
         do iisl=1,nisl
ccc          call pgnumb(iisl,0,1,str3,nc)
ccc          call pgtext(xic(iisl)*1.0-0.5,yic(iisl)*1.0-0.5,str3)
         end do
ccc         call pgsci(1)
        end if
