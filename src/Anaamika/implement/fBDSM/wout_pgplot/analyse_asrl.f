...
ccc        call pgvport(0.1,0.50,0.1,0.4)
c        mny=-0.00759
c        mxy=0.139
ccc        call pgwindow(mnx,mxx,mny,mxy)
c         write (*,*) 'rms win ',mny,mxy
ccc        call pgbox('BCNSTL',0.0,0,'BCNST',0.0,0)
        do i=1,nbin
         x4(i)=log10(x4(i))
        end do
ccc        call pglabel(xlab(1:nchar(xlab)),'Error/Mean',' ')
ccc        call pgpoint(nbin,x4,y4,17)
...
ccc        call pgnumb(abs(round(c1*1.d4)),-4,1,sc1,nc)
ccc        call pgnumb(abs(round(c2*1.d4)),-4,1,sc2,nc)
ccc        call pgsch(0.8)
ccc        call pgtext(mnx+0.3*(mxx-mnx),mny+0.7*(mxy-mny),
     ...
     /        '\\d\\gs\\drms\\u\\u2\\d/S\\u2\\d)\\u0.5\\d)'
ccc        call pgtext(mnx+0.3*(mxx-mnx),mny+0.6*(mxy-mny),stuff)
ccc        call pgsch(1.0)
...
ccc        call pgvport(0.59,0.99,0.1,0.4)
c        mny=-10.061
...
ccc        call pgbox('BCNSTL',0.0,0,'BCNST',0.0,0)
ccc        call pglabel(xlab(1:nchar(xlab)),'Mean',' ')
ccc        call pgpoint(nbin,x4,y4,17)
...
ccc        call pgsci(5)
ccc        call pgline(100,x4,y4)
        dumr4=log10(tpi)
ccc        call pgmove(dumr4,mny)
ccc        call pgdraw(dumr4,mxy)
ccc        call pgsci(1)
...
ccc        call pgvport(0.65,0.95,0.51,0.99)
ccc        call pgwindow(0.0,1.0,0.0,1.0)
ccc        call pgsch(1.1)
ccc        call pgtext(0.0,0.9,'File : '//scrat(1:nchar(scrat)))
ccc        call pgtext(0.0,0.8,'Pink/green : Mean; '//
 
        tol4=tol
        call get_pgnumb(tol4,mm,pp)
ccc        call pgnumb(mm,pp,1,str1,nc)
ccc        call pgtext(0.0,0.7,'Association search radius : '//
