...
c! CHANGE SRL FORMAT
     ...
     /         crval,crota,bm_pix,scratch)
         ...
         n_s=min(int(bm_pix(1))*10,n)
         m_s=min(int(bm_pix(2))*10,m)
         l_s=l0
...
c! actually, no ... each image wont be exactly coincident due to ionosphere ...
     ...
     /      crpix,cdelt,crval,crota,bm_pix,n_s,m_s,l_s)
        ...
        character code(nsrc)*4,mean_map*500
        integer wcslen,error,delx,dely,delz,dumi1,dumi2
...
c! get rms and mean image
        call get_rmsmap(fullname,n1,m1,rmsim)
        call get_avmap(fullname,n1,m1,avim)
           ...
           if (peak1ch/std0.ge.thresh_sp2) then
            ...
            do j=int(-bm_pix(2)*1.5),int(bm_pix(2)*1.5)  ! 1.5 is a hack
             do ii=int(-bm_pix(1)*1.5),int(bm_pix(1)*1.5)  ! 1.5 is a hack
              if (srcx(i)+ii.ge.1.and.srcy(i)+j.ge.1.and.
     /            srcx(i)+ii.le.n1.and.srcy(i)+j.le.m1) then
            ...
            delz=0
            dumi1=min(n,int(srcx(i)+diffpix))-delx!+1
            dumi2=min(m,int(srcy(i)+diffpix))-dely!+1
            do k=1,l_s
             do j=1,dumi2
              do ii=1,dumi1
