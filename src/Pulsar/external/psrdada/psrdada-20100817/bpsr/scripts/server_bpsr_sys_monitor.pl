...
use constant PIDFILE     => "bpsr_sys_monitor.pid";
use constant QUITFILE    => "bpsr_sys_monitor.quit";
use constant LOGFILE     => "nexus.sys.log";
...
$server_socket = new IO::Socket::INET (
    ...
    LocalPort => $cfg{"SERVER_SYS_LOG_PORT"},
    Proto => 'tcp',
    Listen => 1,
    Reuse => 1,
);
...
debugMessage(2, "Waiting for connection on ".$cfg{"SERVER_HOST"}.":".$cfg{"SERVER_SYS_LOG_PORT"});
...
while (!$quit_daemon) {
  ...
  foreach $rh (@$readable_handles) {
    ...
    if ($rh == $server_socket) { 
      ...
      $read_set->add($handle); 
      debugMessage(2, "Adding connection: (".$hostname."): ".$handle);
      ...
      if (! defined $string) {
        $read_set->remove($rh);
        debugMessage(2, "Removing connection: ".$rh);
        close($rh);
...
sub logMessage($$) {
  ...
  # This daemon will only handle SYS messages
  if ($type eq "sys") {

    # Log file for this message
    $logfile = $logfile_dir."/".$machine.".sys.log";

    if ($level eq "WARN") {
      $statusfile = $statusfile_dir."/".$machine.".sys.warn";
    } elsif ($level eq "ERROR") {
      $statusfile = $statusfile_dir."/".$machine.".sys.error" ;
    } elsif ($level eq "INFO")  {
      ;
    } else {
      $logfile = "";
