...
int main(int argc, char *argv[])
{
  ...
  if(timestamp.tv_usec < 0) 
	...
	// read the source header file
	if (read_source_file(argv[3], &source, &BW, source_name ) < 0)
		return EXIT_FAILURE;
...
{
  fprintf (stdout,
     "Usage: gmrt_delay antsys sampler source [offset]\n"
     " antsys         antenna definition file\n"
		 " sampler        antenna list file [for query]\n"
		 " source         source definition file\n"
