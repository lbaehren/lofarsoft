...
c
      ...
      character*70 string(8),period,acceln,accdot,dmidx*4,dmval*8
     ...
     &     parmin(2), parmax(2), grid(mg,mg), tr(6), xmin, xmax,
     &     xval(mg), yval(mg), gridmin, gridmax, deltax, deltay,
     &     xx, yy, fcentre, chbandw

      integer trid(mc),cidx(mc),i,j,k,l,sidx(mc),nids(mc),ndm
      integer narg,iargc,fld(mc),nsus,nf,mode,ix,iy,gidx
...
c               stop 'HELP!'
            ...
            if (lview) then
               if (mode.ne.4) then
                  ...
                  write(*,6) par(j),snr(j),ac(trid(j)),ad(trid(j))
               endif
            endif
            nsus=nsus+1
c
...
c
            call glun(lsum)
            if (nsus.lt.10) then
               write(susfile,'(a,i1,a)')'sus',nsus,'.sum'
            else if (nsus.lt.100) then
               write(susfile,'(a,i2,a)')'sus',nsus,'.sum'
            else
            ...
            if (mode.eq.4) then
               ...
               write(lsum,*) par(j),snr(j),dd(trid(j))
            endif
            ndm=0
            snrmax=0.0
            do k=1,nc
     ...
     &               tr(4)+real(iy)*tr(6),grid(ix,iy)
                     ...
                     tr(1)=parmin(1)-deltax
                     tr(2)=deltax
                     tr(3)=0.0
                     tr(4)=parmin(2)-deltay
                     tr(5)=0.0
                     tr(6)=deltay
                  else
