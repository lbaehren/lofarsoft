/*
  ...
  The inputs to this subroutine include. 
   ...
   'iterate' - If > 0, use iterative approach to finding pulses (i.e. 
	       mean and rms are calculated, pulses are found, and then 
               mean and rms recalculated with those pulses removed.
...
int pulse_(realdata, thresh, iterate, nsmax, ndm, length, data, loopsize, scrdsk1, lsd1)
int *iterate, *nsmax, *length, *ndm, *loopsize, *lsd1;
char scrdsk1[];
