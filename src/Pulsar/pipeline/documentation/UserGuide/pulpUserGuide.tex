...
\verb|${LOFARSOFT}/src/Pulsar/pipeline/documentation|\footnote{This
  document can be found op ``UserGuide/UserGuide.pdf','' under ``documentation/''}Further documentation on the LOFAR pipeline framework, development
...
$LOFARSOFT src/Pulsar/pipeline/documentation:
  ...
  drwxr-xr-x   6 <user>     staff  204 Feb  3 15:09 Pulp_API
...
so-called  `incoherentstokes' beam-formed data.\footnote{JH mandated that
development on `coherentstokes' data processing would be tabled until
the data type was being written to the cluster in conformance with the
LOFAR-USG-ICD-003 specification}\footnote{see LOFAR document, \textit{LOFAR-USG-ICD-003,
