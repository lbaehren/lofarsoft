...
document.\footnote{Readers of this
  document finding themselves unfamiliar with the terms, `known pulsar
...
\label{sec:package}
The Pulp package is a framework implementation of the LOFAR TKP Pulsar Group's pipeline shell 
script\footnote{ \texttt{make\_subs\_SAS\_Ncore\_Mmodes.sh}, src:
...
shell script\footnote{The framework and Pulp are node generic.}.
...
\item Pulp accomodates any number of observation subbands and any
  ...
  from a few subbands\footnote{esp., ``coherentstokes''}, to the usual
  248.  However, Pulp places no constraint on the number of subbands an
  observation may comprise.
