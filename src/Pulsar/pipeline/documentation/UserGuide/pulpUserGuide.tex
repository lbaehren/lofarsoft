...
pipeline definition can be thought of as writing your own interface to
control what processing is done  Users should be aware that, though
the recipes are ``independent'', there is usually a proxy dependence
in that they do depend upon the state of the data on disk as it may be provided by
recipes executed prior.} , that is, their own pipeline
definitions. The two defintions provided are
\begin{enumerate}
\item \verb|pulp.py|
...
describe during the early November ``pulsar busy week''.  This
...
The recipe stack is noticeably smaller, sans prepfold. 
\begin{verbatim}
Current usage:
...
filefactor  = subband splitting factor, <int>
              ...
              default = 8
\end{verbatim}
Users will find the data products of \verb|dynspec.py| located in the
