...
part of any download of the LOFAR software repository.\footnote{Publicly available for download with \texttt{svn co http://usg.lofar.org/svn/code/trunk your/lofarsoft/path/here}. This path will be set to envar \texttt{\$LOFARSOFT.}}
...
executable ``pulp.py'' module is invoked in the common pythonic idiom,\\
 ...
 i.e. \verb|$ python pulp.py -d --job-name ...|\footnote{``\texttt{Nsubbands}'' is this example represents the number of subband data files comprising ``this''  pipeline run.}\\
\begin{verbatim}
 Current usage:
...
filefactor  = subband splitting factor, <int>
              user speciﬁcation (range,1--Nsubbands),
              default = 8
\end{verbatim}
