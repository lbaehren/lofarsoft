...
OBSID=L2010_06213
PARSET=/globalhome/lofarsystem/log/L2010_06213/RTCP.parset.0
DISK=data2
PULSAR=B1919+21
...
bf2presto ${COLLAPSE} -A 10 -f 0 -c ${CHAN} -n ${DOWN} -N ${SAMPLES} -o ${STOKES}/RSP0/${PULSAR}_${OBSID}_RSP0 `ls $RSP0 | sort -t B -g -k 2` >> ${STOKES}/RSP0/bf2presto_RSP0.out 2>&1
echo `date` "RSP0 Done"
#RSP1 conversion
echo 'Converting subbands: '`ls $RSP1` >> ${STOKES}/RSP1/bf2presto_RSP1.out 2>&1
bf2presto ${COLLAPSE} -A 10 -f 0 -c ${CHAN} -n ${DOWN} -N ${SAMPLES} -o ${STOKES}/RSP1/${PULSAR}_${OBSID}_RSP1 `ls $RSP1 | sort -t B -g -k 2` >> ${STOKES}/RSP1/bf2presto_RSP1.out 2>&1
echo `date` "RSP1 Done"
#RSP2 conversion
echo 'Converting subbands: '`ls $RSP2` >> ${STOKES}/RSP2/bf2presto_RSP2.out 2>&1
bf2presto ${COLLAPSE} -A 10 -f 0 -c ${CHAN} -n ${DOWN} -N ${SAMPLES} -o ${STOKES}/RSP2/${PULSAR}_${OBSID}_RSP2 `ls $RSP2 | sort -t B -g -k 2` >> ${STOKES}/RSP2/bf2presto_RSP2.out 2>&1
echo `date` "RSP2 Done"
#RSP3 conversion
echo 'Converting subbands: '`ls $RSP3` >> ${STOKES}/RSP3/bf2presto_RSP3.out 2>&1
bf2presto ${COLLAPSE} -A 10 -f 0 -c ${CHAN} -n ${DOWN} -N ${SAMPLES} -o ${STOKES}/RSP3/${PULSAR}_${OBSID}_RSP3 `ls $RSP3 | sort -t B -g -k 2` >> ${STOKES}/RSP3/bf2presto_RSP3.out 2>&1
echo `date` "RSP3 Done"
