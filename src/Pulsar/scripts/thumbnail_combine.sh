...
find ./ -iname "*.th.png" ! -iname "*status*png" ! -iname "*_nomask*png" -print | sort -d > /tmp/$$_combine_col1.txt
...
find ./ -iname "*prepout" ! -iname "*_nomask*" -print -exec grep -i squared {} \; | sed 's/prepout/prepout \\/g' | sed -e :a -e '/\\$/N; s/\\\n//; ta' | sort -d | sed 's/^.*= //g'  > /tmp/$$_combine_col3.txt
...
find ./ -iname "*.th.png" ! -iname "*status*png" ! -iname "*_nomask*.th.png" -print | sort -d | sed -e "s/\// /g" -e "s/_/ /g" -e "s/incoherentstokes/ IS_/g" -e 's/stokes/ CS_/g' -e "s/\.pfd.*//g" -e "s/\.//g" -e "s/_ .*L20.. /_ /g" -e "s/^.* L20.. / CS_ /g" -e "s/RSP/_RSP/g" | awk '{if (NF == 7) print $1$2"\\n_"$7; else if (NF == 5) print $1"_"$3"\\n"$5;  else print $1"_"$2"\\n_"$5}' > /tmp/$$_combine_col2.txt

find ./ -iname "*.th.png" ! -iname "*status*png" ! -iname "*_nomask*.th.png" -print | sort -d |  sed -e "s/^.*beam/beam/g" -e "s/\/.*//g" -e "s/\.//g" | awk '{print "\\n"$1}' > /tmp/$$_combine_col4.txt
