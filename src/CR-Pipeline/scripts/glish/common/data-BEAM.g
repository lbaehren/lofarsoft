#######################################################################
#
# Functions and other stuff for beamed data
#
########################################################################

#-----------------------------------------------------------------------
#Name: BEAM(gen|conv)ZZZ
#-----------------------------------------------------------------------
#Type: Function
...
#-----------------------------------------------------------------------
BEAMgetDummy := function(ref ary,ref rec,index=1,inv=F, reffield='',start=-1,end=-1,...) {
  global globalpar
  globalpar.lastaction:=spaste('Reading from Disk',rec.head('Files',index));
  ofs:=rec.getmeta('Offset',index)*2
#print 'BEAMgetDummy: Offset:',rec.getmeta('Offset',index),'Samples ->',ofs,'Bytes'
#print 'BEAMgetDummy: DataFormat:',rec.head('DataFormat')
#print 'BEAMgetDummy: Reading data...'
#print 'BEAMgetDummy: samplerate:',samplerate,rec.head('Samplerate')
  return readdat(rec.head('Files',index),offset=ofs,blocksize=rec.getmeta('Blocksize',index),type=rec.head('DataFormat'))
};
BEAMconvDummy2fx := function(ref ary, ref rec,index=1,inv=F, reffield='',...) {
  #return  rec.flagdat(sqrt_signed(ary),'f(x)',index)
  return  rec.flagdat(ary,'f(x)',index)
};
BEAMconvVolt2RawFFT := function(ref ary, ref rec,index=1,inv=F, reffield='',...) {
...
BEAMconvIF2Frequency := function(ref ary, ref rec,index=1,inv=F,...) {
  #both directions (IF -> Freq & Freq -> IF) are the same
  return ary+(rec.headf("Frequency",index))
};
BEAMconvRawFFT2FFT := function(ref ary, ref rec,index=1,inv=F, reffield='',...)
{
  return rec.flagdat(ary,'FFT',index)
};

#-----------------------------------------------------------------------
#Name: beamgenEmptyRec
#-----------------------------------------------------------------------
#Type: Function
#Doc: Creates the data field records and definiton for the different
#     fields for the beamed data
#
#Ret: Record with fields for vital header information
#-----------------------------------------------------------------------
BEAMgenEmptyRec := function() {
	ci := [=];
	...
	ci['Dummy']:=[Units='Counts', DefAxis='y', Domain='T', Stored=F, 
		Ref='-', Type='unreal',GenFunc=symbol_value('BEAMgetDummy'),
		IsInput=T];
	ci['f(x)']:=[Units='Counts', DefAxis='y', Domain='T', Stored=T, 
		Ref='Dummy', Type='real',ConvFunc=symbol_value('BEAMconvDummy2fx'),
		IsInput=F];
