...
double ShapeletsModel1D::center ()
{
  return center_p;
}
...
vector<double> ShapeletsModel1D::coefficients ()
{
...
double ShapeletsModel1D::eval (const double x)
{  
  int nofCoeff;
  double xx;
  double y;
  ...
  // evaluate the model
  for (int n=0; n<nofCoeff; n++) {
    y += coefficients_p[n]*Shapelets1D::eval(n,xx);
  }
  
  return y;	
}

vector<double> ShapeletsModel1D::eval (const vector<double>& x)
{  
  int nelem (x.size());      // Number of function arguments
  vector<double> y (nelem);  // Vector with the function values
  
  for (int i=0; i<nelem; i++) {
    y[i] = ShapeletsModel1D::eval(x[i]);
  }
