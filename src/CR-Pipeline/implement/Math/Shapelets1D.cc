...
Shapelets1D::Shapelets1D (int order)
  : Hermite1D (order)
{
  Shapelets1D::setBeta ();
}

Shapelets1D::Shapelets1D (int order, double beta)
  : Hermite1D (order)
{
...
double Shapelets1D::eval (int l,
			  double x)
{
  double hermite;  // Value of the Hermite polynomial of order n
  double phi;      // Value of the Shapelet of order n (return value)
  double xx;       // Argument for the exponential function
...
double Shapelets1D::integral (const int n)
{
  // is the requested value in cache?
  if (n > order_p) {
    Shapelets1D::setOrder (n);
  }
