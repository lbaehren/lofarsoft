...
int testfun (string infile2, string outfile, int maxblock, int startblock=0, int integrationlength=1)
{
	//int integrationlength=1;
	cerr << "Starting tesfun\n";
	int nofFailedTests =0;
	int ninputfiles=11;
	int nrCHANNELS = ninputfiles*CHANNELS;
	FILE ** pFile;
	pFile = new FILE*[ninputfiles];	
	std::string* infile;
	infile=new std::string[ninputfiles];
	infile[0]="/Users/STV/Astro/data/pulsars/L2009_13298/SB0.MS";
	infile[1]="/Users/STV/Astro/data/pulsars/L2009_13298/SB1.MS";
	infile[2]="/Users/STV/Astro/data/pulsars/L2009_13298/SB2.MS";
	infile[3]="/Users/STV/Astro/data/pulsars/L2009_13298/SB3.MS";
	infile[4]="/Users/STV/Astro/data/pulsars/L2009_13298/SB4.MS";
	infile[5]="/Users/STV/Astro/data/pulsars/L2009_13298/SB5.MS";
	infile[6]="/Users/STV/Astro/data/pulsars/L2009_13298/SB6.MS";
	infile[7]="/Users/STV/Astro/data/pulsars/L2009_13298/SB7.MS";
	infile[8]="/Users/STV/Astro/data/pulsars/L2009_13298/SB8.MS";
	infile[9]="/Users/STV/Astro/data/pulsars/L2009_13298/SB9.MS";
	infile[10]="/Users/STV/Astro/data/pulsars/L2009_13298/SBX.MS";
	...
	for(int i=0;i<ninputfiles;i++){
		...
		shortinfile[i] = std::string(infile[i],42,3);
	}
...
//	}
	...
	for(int i=0;i<nrCHANNELS;i++){fw[i]=0; bw[i]=0;}
	...
	short *fitsdata = (short*)malloc(totSamples * (nrCHANNELS+2) * sizeof(short));
	...
	for(int j=0;j<totSamples;j++){
		for(int i=0;i<nrCHANNELS+2;i++){
			fitsdata[j*(nrCHANNELS+2)+i]=0;
		}
	}
	...
	for(int i=0;i<ninputfiles;i++){
	   fseek(pFile[i], startblock*(sizeof data[i]), SEEK_SET);
	}
    ...
    for(int k=0;k<kmax;k++){ 
		...
		for(int i=0;i<ninputfiles;i++){
			...
			num = fread( &data[i], sizeof data[i], 1, pFile[i]);
			if( !num) {
				cout << "Error, EOF reached.\n";
			}
			
			if(i==0){// && data[i].sequence_number%10==0){  
				swap_endian( (char*)&(data[i].sequence_number) );
				cerr << "sequence number: " << data[i].sequence_number << endl;
			}
		}
		...
		for(int time=0;time<SAMPLES;time++)
		{
			...
			for(int sb=0;sb<ninputfiles;sb++){
				for(int channel=0;channel<CHANNELS;channel++){
					//cout << k << " " << time << " " << sb << " " << " " << channel << endl;
					totchannel=sb*CHANNELS+channel;
					//float value = 1;
					float value =  data[sb].samples[channel][time] ; //const float before
				...
				// if isnan value --> value is zero, add counters for how many good things there are
					...
					if(value == 0) { zerocounter++; cout << "zero found on time: "<< k*SAMPLES+time << " in channel " << totchannel << endl;}
					channelsum += value;
				
					rest=(k*SAMPLES+time)%nrCHANNELS;
//					if(totchannel==0){
...
//					}
//				else { //if(value!=0) {
//					fw[(rest-totchannel)%nrCHANNELS]+=value;
//					//fwcounter[(rest-channel)%64]++;
//				}
					if(totchannel==nrCHANNELS-1){
						if(bw[rest]>bwmaximum){
							bwmaximum=bw[rest]; 
						//cout << "new backward maximum: " <<bwmaximum << endl;
						}
						bin = (int) bw[rest]/av/bin_width;
						if(time%100==0){cout << time << " " << bin << endl;}
					//cout << sb << " " << channel << endl;
...
//						bin = (int) sumsubsumbw/av/bin_width;
						...
						else if(k>1 || time >= nrCHANNELS+integrationlength-1){  fitsdata[totaltime + totSamples*(nrCHANNELS+1)] = (short) bin;  }
						bw[rest]=value;
						
					
					}
					else { //if(value!=0) {
						bw[(rest-(nrCHANNELS-1)+totchannel)%nrCHANNELS]+=value;
...
*/
	...
	int status            = 0;
	long naxis            = 2;
	uint nelements        = (nrCHANNELS+2)*(totSamples);
	long naxes[2]         = { totSamples, nrCHANNELS+2};
	long firstelement = 1;
