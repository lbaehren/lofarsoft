		  ...
		  char *argv[])
{
	...
	std::string infile, outfile="TBBskymapper.h5";
	 ...
	 */
	if (argc < 2) {
		std::cout << "Usage: testnewskymapping <inputfile.dat> [<output-image>]. Now using /data/TBBskymapper.dat" << endl;
		infile = "../../../src/CR-Tools/data/TBBskymapper.dat"; 
	} else {
		infile = argv[1];
		if (argc > 2) {
			outfile  = argv[2];
	...
	try {    
		...
		for(int i=0; i<ninputfiles; i++){
			...
			if(arrayspread=="inner"){
				...
				tempantPositions = antenna_position_value(used_rcu_ids[i], station_id, "HBA");
