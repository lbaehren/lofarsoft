...
//	if (argc < 2) {
//		std::cout << "Usage: fitsTOstokes [<inputfile.MS>] [nrblocks to process] [startblock] [<length of integration>] [firstSB] [lastSB] [#channels] [#samples] [use padding 0/1] [file extension] \n Now using input /Users/STV/Astro/data/pulsars/L2009_13298/*.MS \n and output /Users/STV/Documents/GiantPulse/"<< endl;
//	} else {
//		infile = argv[1];
//		if (argc > 2) {
...
//	}
	int nrfreqs = 32769;
	int nrtimes= 1000;
	std::string filename="/Users/STV/Astro/data/crab_pulse1_float_spec.dat";
	FILE *pFile;
	...
	float *data=(float*)malloc(nrtimes*nrfreqs);
	if (data==NULL)
	{
		cerr << "Memory could not be allocated\n";
		return 1;
	}
	data = new float[nrtimes*nrfreqs];
	int datasize=nrtimes*nrfreqs*sizeof(float);
...
//		cout << " " << data1 << std::endl;
//	}
	num = fread( data, datasize, 1, pFile);
	
	cout << "data" << data[0] << "\t" << data[1] << "\t" << data[nrfreqs] << "\t" << data[nrfreqs+1] << std::endl;
	if( !num) {
				cout << "Error, EOF reached.\n";
	}
	...
	stokesdata *sdata;
	int fSB=0;
	int lSB=3;//nrfreqs/CHANNELS;
	int nrblocks=nrtimes/SAMPLES;
	nrblocks=1;
	//sdata.sequence_number=0;
	std::string outfile3;
	std::string outfiledir="/Users/STV/Astro/data/stokes/";
	//std::cout << "writing data to " << outfiledir << std::endl;
	for(int i=fSB; i<lSB; i++){
		std::stringstream ss;
	    std:string number;
		ss << i;
		ss >> number;
		outfile3=outfiledir + "SB" + number + ".MS" + extension;
		FILE *oFile;
		oFile = fopen(outfile.c_str(),"wb");
		
		for(int bl=0;bl<nrblocks;bl++){
			sdata->sequence_number=bl;
			for(int ch=0;ch<CHANNELS;ch++){
				for(int sa=0;sa<SAMPLES;sa++){
					sdata->data[ch][sa]=data[sa*nrfreqs+ch];
				}
			}
			num = fwrite( &sdata, sizeof(stokesdata), 1, oFile);
			if (!num) {
