...
Double_t data_x[FreqBands][TotalNumbers],data_y[FreqBands][TotalNumbers],
         ...
         data_x_3[FreqBands][TotalNumbers],data_y_3[FreqBands][TotalNumbers], Average_Power[FreqBands];     

Int_t number_lines=0, ndim[1], ndim2[1], leave;

TVirtualFFT *fftx, *ffty, *rft;

char outFileNameR[256] ="analyses_4freq.root";
...
TH1D *Px_raw[FreqBands];
TH1D *Py_raw[FreqBands];
TH1D *Px[FreqBands];
TH1D *Py[FreqBands];
TH1D *TotalPower[FreqBands];
TH1D *TotalPowerAll = new TH1D("TotalPower","TotalPower",1000,0,10000); 
TH1D *Phases_x[FreqBands]; //added 04/02
TH1D *Phases_y[FreqBands];
TH1D *Power_Average_Ratio[FreqBands];
...
void FFTChannel(int freq, int filestep, int step_aver_fft)
{
	...
	if (myMoon.unpackEventC())
        {
		 ...
		 Px_raw[freq]->Fill(Signalx[event]); //Do it here to get original data in Pxy
		 Py_raw[freq]->Fill(Signaly[event]);
	}
	else onereadfailed = 1;

	fftx->SetPoint(event, Signalx[event]/12.); //Division by 12. to make comparison with old data possible.
	ffty->SetPoint(event, Signaly[event]/12.);
	...
	Phases_x[freq]->Fill(Phasex[binx]); //added 04/02
	Phases_y[freq]->Fill(Phasey[binx]);
	
    }

    //for (int j=1;j<26;j++)
    //  {	 
    // 	  Dynamic_phaseX[freq][j][step_aver_fft]=2.0*pi*rand()/RAND_MAX; //added as test 04/02
    // 	  Dynamic_phaseY[freq][j][step_aver_fft]=2.0*pi*rand()/RAND_MAX;
    //  } 
    // Dynamic_ampX[freq][0][step_aver_fft]=0;
    // Dynamic_ampY[freq][0][step_aver_fft]=0;
}
...
void RemoveRFIFreqChannel(int freq, int filestep)
{
      ...
      thresholdrfi_0[1] = 10.0;  //original 10.0
      thresholdrfi_0[2] = 5.0; //original 5.0
      ...
      for (Int_t iterations = 0; iterations < 2; iterations++)	
	{
	  ...
	  TF1 *f1 = new TF1("f1","pol9",0.00005,0.5); //origineel 0.001 ipv 0.00005
	  TF1 *f2 = new TF1("f2","pol9",0.00005,0.5);
		...
		{     
		  ...
		  if (candidates_x[i]>0)
		      ...
		      for (int j = from; j < to+1; j++)
			{
		      ...
		      Dynamic_ampX[freq][i][step_aver_fft] = average_x; //This is the actual cutting of peaks
		      //Dynamic_phaseX[freq][i][step_aver_fft] = 2.0*pi*rand()/RAND_MAX; //changed 04/02
		  ...
		  if (candidates_y[i]>0)
		      ...
		      for (int j = from; j < to+1; j++)
			{
		      ...
		      Dynamic_ampY[freq][i][step_aver_fft] = average_y;  //This is the actual cutting of peaks
		      //Dynamic_phaseY[freq][i][step_aver_fft] = 2.0*pi*rand()/RAND_MAX; //changed 04/02
...
void FillPower(int freq)
{
	...
	for(Int_t step2_0 = step; step2_0 < step + 5; step2_0++)
	{
        ...
        Average_Power[freq]+=Power2;
    }   
    
    Average_Power[freq]/=(TotalNumbers-5);
    
    for(Int_t i=0;i<TotalNumbers-5;i++)
    {
       Power_Average_Ratio[freq]->Fill(Power[freq][i]/Average_Power[freq]);
    }
}

void analyses_new_v20(Double_t TEC = 0.0, Double_t gain_gate_x_0 = 10.0, 
		      Double_t gain_gate_y_0 = 10.0, Double_t gain_gate_x_1 = 10.0,
		      Double_t gain_gate_y_1 = 10.0, Double_t gain_gate_x_2 = 10.0, Double_t gain_gate_y_2 = 10.0, 
		      Double_t gain_gate_x_3 = 10.0, Double_t gain_gate_y_3 = 10.0)
{
...
//Skipped Stopwatches here
  ...
  Double_t gain_gate_y[4] = {10.0,10.0,10.0,10.0}; 
    ...
    gain_gate_x[0]=gain_gate_x_0;
    gain_gate_y[0]=gain_gate_y_0;
    gain_gate_x[1]=gain_gate_x_1;
    gain_gate_y[1]=gain_gate_y_1;
    gain_gate_x[2]=gain_gate_x_2;
    gain_gate_y[2]=gain_gate_y_2;
    gain_gate_x[3]=gain_gate_x_3;
    gain_gate_y[3]=gain_gate_y_3;

 
  sprintf(inFileName[0], "/mnt/lofar/numoon/raw_data_23Mar/storage05/MOON22.000000.dada");
  sprintf(inFileName[1], "/mnt/lofar/numoon/raw_data_23Mar/storage04/MOON22.000000.dada");
  sprintf(inFileName[2], "/mnt/lofar/numoon/raw_data_23Mar/storage03/MOON22.000000.dada");
  sprintf(inFileName[3], "/mnt/lofar/numoon/raw_data_23Mar/storage02/MOON22.000000.dada");
  sprintf(outFileName[0], "dada.output.1");
  sprintf(outFileName[1], "dada.output.2");
  sprintf(outFileName[2], "dada.output.3");
  sprintf(outFileName[3], "dada.output.4");
  sprintf(outFileName4, "dada.time.output");
  sprintf(outFileNameR, "analyses_4freq.root"); 
  ...
  ProjectionX[0] = new TH1D("ProjectionX_0","ProjectionX_0",TotalNumbers/2,-0.000025,0.499975); //added a 0 and a 9 (and below)
  ProjectionY[0] = new TH1D("ProjectionY_0","ProjectionY_0",TotalNumbers/2,-0.000025,0.499975);
  ProjectionX[1] = new TH1D("ProjectionX_1","ProjectionX_1",TotalNumbers/2,-0.000025,0.499975);
  ProjectionY[1] = new TH1D("ProjectionY_1","ProjectionY_1",TotalNumbers/2,-0.000025,0.499975);
  ProjectionX[2] = new TH1D("ProjectionX_2","ProjectionX_2",TotalNumbers/2,-0.000025,0.499975);
  ProjectionY[2] = new TH1D("ProjectionY_2","ProjectionY_2",TotalNumbers/2,-0.000025,0.499975);
  ProjectionX[3] = new TH1D("ProjectionX_3","ProjectionX_3",TotalNumbers/2,-0.000025,0.499975);
  ProjectionY[3] = new TH1D("ProjectionY_3","ProjectionY_3",TotalNumbers/2,-0.000025,0.499975);

  TotalProjectionX[0] = new TH1D("TotalProjectionX_0","TotalProjectionX_0",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY[0] = new TH1D("TotalProjectionY_0","TotalProjectionY_0",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX[1] = new TH1D("TotalProjectionX_1","TotalProjectionX_1",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY[1] = new TH1D("TotalProjectionY_1","TotalProjectionY_1",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX[2] = new TH1D("TotalProjectionX_2","TotalProjectionX_2",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY[2] = new TH1D("TotalProjectionY_2","TotalProjectionY_2",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX[3] = new TH1D("TotalProjectionX_3","TotalProjectionX_3",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY[3] = new TH1D("TotalProjectionY_3","TotalProjectionY_3",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX_bgfree[0] = new TH1D("TotalProjectionX_0_bgfree","TotalProjectionX_0_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY_bgfree[0] = new TH1D("TotalProjectionY_0_bgfree","TotalProjectionY_0_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX_bgfree[1] = new TH1D("TotalProjectionX_1_bgfree","TotalProjectionX_1_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY_bgfree[1] = new TH1D("TotalProjectionY_1_bgfree","TotalProjectionY_1_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX_bgfree[2] = new TH1D("TotalProjectionX_2_bgfree","TotalProjectionX_2_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY_bgfree[2] = new TH1D("TotalProjectionY_2_bgfree","TotalProjectionY_2_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionX_bgfree[3] = new TH1D("TotalProjectionX_3_bgfree","TotalProjectionX_3_bgfree",TotalNumbers/2,-0.000025,0.499975);
  TotalProjectionY_bgfree[3] = new TH1D("TotalProjectionY_3_bgfree","TotalProjectionY_3_bgfree",TotalNumbers/2,-0.000025,0.499975);
  ...
  Power_Average_Ratio[0] = new TH1D("PA_ratio_0","PA_ratio_0",1000,0,50);
  Power_Average_Ratio[1] = new TH1D("PA_ratio_1","PA_ratio_1",1000,0,50);
  Power_Average_Ratio[2] = new TH1D("PA_ratio_2","PA_ratio_2",1000,0,50);
  Power_Average_Ratio[3] = new TH1D("PA_ratio_3","PA_ratio_3",1000,0,50);
  
  Phases_x[0] = new TH1D("Phases_x_0","Phases_x_0",100,-pi,pi); //added 04/02
  Phases_x[1] = new TH1D("Phases_x_1","Phases_x_1",100,-pi,pi);
  Phases_x[2] = new TH1D("Phases_x_2","Phases_x_2",100,-pi,pi);
  Phases_x[3] = new TH1D("Phases_x_3","Phases_x_3",100,-pi,pi);
  Phases_y[0] = new TH1D("Phases_y_0","Phases_y_0",100,-pi,pi);
  Phases_y[1] = new TH1D("Phases_y_1","Phases_y_1",100,-pi,pi);
  Phases_y[2] = new TH1D("Phases_y_2","Phases_y_2",100,-pi,pi);
  Phases_y[3] = new TH1D("Phases_y_3","Phases_y_3",100,-pi,pi);
  

  Px_raw[0] = new TH1D("Px_0_raw","Px_0_raw",1000,-256,256); //added 04/02
  Py_raw[0] = new TH1D("Py_0_raw","Py_0_raw",1000,-256,256);
  Px_raw[1] = new TH1D("Px_1_raw","Px_1_raw",1000,-256,256);
  Py_raw[1] = new TH1D("Py_1_raw","Py_1_raw",1000,-256,256);
  Px_raw[2] = new TH1D("Px_2_raw","Px_2_raw",1000,-256,256);
  Py_raw[2] = new TH1D("Py_2_raw","Py_2_raw",1000,-256,256);
  Px_raw[3] = new TH1D("Px_3_raw","Px_3_raw",1000,-256,256);
  Py_raw[3] = new TH1D("Py_3_raw","Py_3_raw",1000,-256,256);
  ...
  for (int filestep = 0; filestep < FileTotal/2; filestep++)
  {
    cout << filestep << endl;
      leave=0;
	 ...
	 for (int step_aver_fft = 0; step_aver_fft < NumberOfSpectra; step_aver_fft++)
         ...
         if (cal_fact_x > gain_gate_x[freq]) {leave=1;break;}  //go out of freq-loop
         if (cal_fact_y > gain_gate_y[freq]) {leave=1;break;}
      }

      if (leave==1) {continue;} //go to next filestep
	  ...
	  for (Int_t step1_0 = 0; step1_0 < TotalNumbers - 5; step1_0++)
	    {
	      //if(Power[0][step1_0] > Threshold0)
                if(Power[0][step1_0] > 2.8*Average_Power[0])
		{
		  number_events_band1++;
		  if (rft_band1 == 0)
		    {
...
//		      if(Power[1][margin_1] > Threshold1)
                      if(Power[1][margin_1] > 2.8*Average_Power[1])
...
//			      if(Power_1[2][margin_2] > Threshold2 || Power[2][margin_2] > Threshold2 || Power_3[2][margin_2] > Threshold2)
			      if(Power[2][margin_2] > 2.8*Average_Power[2])
...
//				      if(Power_1[3][margin_3] > Threshold3 || Power[3][margin_3] > Threshold3 || Power_3[3][margin_3] > Threshold3)					
                                      if(Power[3][margin_3] > 2.8*Average_Power[3])
