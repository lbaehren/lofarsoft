...
//------------------------------------------------------------
// INCLUDE
#include <GlishClients/timebeam.h>
...
cout << "[timebeam] reading raw antenna data..." << endl;
    ...
    char *binfilename = CR::string2char(filename+ending);
...
cout << "[timebeam] looping over blocks... " << "\n" << endl;
    ...
    // progress bar
    CR::ProgressBar bar (nblocks,70); // widht = 70 pixels
    bar.showTime(false);
...
*/
	...
	cerr << "[timebeam@nextdatareaderblock] " << x.getMesg() << endl;
      ...
      Vector<Double> timeblock(tvecshape);
        ...
        try {
      //timeblock = CR::FFT2Fx(freqblock,timelen,False,False); // F/T=freqinv/no, F/T=FFT/inv
