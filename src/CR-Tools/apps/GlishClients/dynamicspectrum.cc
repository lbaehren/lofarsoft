			     ...
			     const Vector<double>& cdelt)
{
  ...
  char *cunit[] = {"s", "Hz"};
  float crpix (1.0);
  char *observer = CR::string2char(clientData->observer);
  char *telescope = CR::string2char(clientData->telescope);
			   ...
			   void *)
{
  ...
  if (refcode == "AZEL") {
    ...
    String Observatory;
    //
    if (!CR::QuantityFromRecord (epoch, "epoch", glishRec)) {
...
bool crosscorrDynspec (GlishSysEvent &event, void *) {
  ...
  CR::CrossCorrelation (crosscorr, dynspec, dynspec, -1);
...
bool cmsFrequency (GlishSysEvent &event, void *)
{
  ...
  for (int t=0; t<shapeDynspec(0); ++t) {
    ...
    cmsFreq(t) = CR::WeightedCenter (frequencies,
				     spectrum);
  }
			    ...
			    void *)
{
  ...
  status = CR::QuantityFromRecord   (obsTime,     "time",         glishRec);
  status = CR::MEpochFromRecord     (date,        "date",         glishRec);
  status = CR::MDirectionFromRecord (j2000,       "j2000",        glishRec);
  status = CR::MDirectionFromRecord (azel,        "azel",         glishRec);
  status = CR::MPositionFromRecord  (observatory, "observatory",  glishRec);
