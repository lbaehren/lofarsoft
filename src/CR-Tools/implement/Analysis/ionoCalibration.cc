...
namespace CR { // Namespace CR -- begin
  ...
  {
    os << "[ionoCalibration] Summary of object properties" << endl;
  }
    ...
    
  {
    ...
    elevation  = sin(geomagLatitude)*sin(declination);
    elevation += cos(geomagLatitude)*cos(declination)*cos(hrAngle);
  ...
  Double ionoCalibration::calcSTECvalue ( const Double& height,
    ...
    
  {
    ...
    Double dem_sqr = (cos(elevation))*(cos(elevation));
    
    Double correction_factor = (R_earth/(R_earth + height))*(R_earth/(R_earth + height));
...
// 	 uint n_elements = relativePhase_diff.nelements() ;
  ...
  Matrix<DComplex>
  ionoCalibration::phaseRECorrection (const Matrix<DComplex>& FFT_data,
				      const Double& hrAngle,
				      const Double& declinationAngle,
				      const Double& geomagLatitude,
				      const Double& height_ionosphere,
    ...
    
  {
    try {
      ...
      // Local variables _________________________
      ...
      // Computation _____________________________

      // compute elevation angle
      elevationAngle = ionoCal.calcElevationAngle (hrAngle,
						   declinationAngle,
						   geomagLatitude);
      // compute STEC value
      STEC_value = ionoCal.calcSTECvalue (height_ionosphere,
					  TEC_value,
					  elevationAngle);
