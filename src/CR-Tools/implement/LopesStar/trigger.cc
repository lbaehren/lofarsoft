...
//        position[i] = (int) mean_pos;
  ...
  if(debug){ 
    cout << endl;
    for(int i=0; i<NoChannels; i++) cout << i << " -- " << position[i] << endl;
  }
...
if(1){
  *threshold = (Mean(new_window_size,tmpTrace) + 4.5*RMS(new_window_size,tmpTrace)); // before 5*RMS  4.5
}
else{
...
bool TriggerCoincidenceHardware(int NoCh, int *position, float *coincidence_time, int NoZero, char **ant_pos, int daq_id){
  ...
  if(debug) cout << "set coinc time = "<< *coincidence_time << " ns" << endl;
  if(debug) cout << "max conic time bins = " << max_bins << endl;
...
bool L0Decide(int NoCh, int window_size, float **trace, char **ant_pos, int daq_id, float *coincidence_time, int NoZeros){
  ...
  int new_window_size = window_size*(NoZeros+1);
  int *Position = new int [NoCh];
  float *threshold = new float [NoCh];
...
float VarPeakWidth(int window_size, float *trace, float fraction, float *RaisingEdge, int *ExpectedPosition, int NoZeros, bool DoRectifier){
  ...
  for(int i=0; i<*ExpectedPosition; i++)
   if(TriggerTrace[*ExpectedPosition-i] <= max_value*fraction) {
     ...
     *RaisingEdge = lower_limit/(float)(NoZeros+1);
     break;
   }
  
  width = (upper_limit - lower_limit)/(float)(NoZeros+1);
...
/**********************************************************************************************/
float VarPeakWidthGausFit(int window_size, float *trace, float fraction, float *RaisingEdge, int *ExpectedPosition, float *Peak, float *Peak_err, int NoZeros, bool DoRectifier){
  float width=0;
  int new_window_size = window_size*(1+NoZeros);
  float *TriggerTrace = new float [new_window_size];
  ...
  TF1 *fit = new TF1("fit","gaus",lower_limit, upper_limit);
  float TimeAxis[new_window_size];
  for(int i=0; i<new_window_size; i++) TimeAxis[i] = i;
  TGraph *graph = new TGraph(new_window_size, TimeAxis, TriggerTrace);

  // Do Gaus Fit to found Peak
  if(fraction < 1.0){
    graph->Fit("fit","RQ");
    //cout << "const = " << fit->GetParameter(0) << endl;
    //cout << "mean  = " << fit->GetParameter(1) << endl;
    //cout << "sigma = " << fit->GetParameter(2) << endl;

    width = sqrt(2*TMath::Log(1./fraction)) * fit->GetParameter(2) /(float)(NoZeros+1);
    *RaisingEdge = (fit->GetParameter(1) - width/2.0) / (float)(NoZeros+1);
    *Peak = fit->GetParameter(0);
    *Peak_err = fit->GetParError(0);
   }
   else{
    width = 0;
    *RaisingEdge = 0;
...
void RectifierHilbertSquare(int window_size, float *trace){
   ...
   for(int i=0; i<new_window_size; i++){
     trace[i] = ( powf(trace_out_hilbert[i],2.) + powf(trace_raw[i],2.) );
   }
...
float GetCoincidenceTime(short int ID, float MaxZenith){
  ...
  if(ID == 19) { cerr << "GetCoincidenceTime: not usable" << endl; baseline = 65;}
