...
following applies to you.
  ...
  + Under Aquamacs (which is a MacOS X native port of Emacs), it has
    ...
    :file:`p4config.py`, locate the line defining :code:`DEFADVICE_OK`,
...
+ For the Python part
  ...
  install``.  If the Python interpreter has a non-standard name or
  ...
  Second, it installs the Python package through the Python standard
...
in the three following sections.  The remaning sections address more
...
these :code:`print` statements display the resultst right within the
...
suggests.  My only reservation is about the Python need for non-mutable keys, something which Emacs does not guarantee.  As
by default, from Lisp to Python, references are transmitted instead of
contents, this would be a possible problem only when an expanded copy
is requested from the Python side.  This would never be a problem going
from Python to Emacs, so far as I understand things now.
