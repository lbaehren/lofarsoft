...
class CrossCorrelateAntennas(tasks.Task):
    ...
    **Description**
    ...
    ``fft_data`` instead and the fft will not be (re-)calculate. Note
    that the function currently uses fftcasa convention!
    ...
    **Example:**
    ...
    parameters=dict(
        ...
        reference_fft_data=p_(lambda self:cr.hArray(complex,[self.dim[-1]/2+1]),
                              doc="FFT of the reference data set",workarray=True),
        fft_data=p_(lambda self:cr.hArray(complex,[self.dim[-2],self.dim[-1]/2+1]),doc="FFT of the input timeseries data",workarray=True),
        fft_reference_data=p_(lambda self:cr.hArray(complex,[self.dim[-1]/2+1]), 
        ...
        crosscorr_data=p_(lambda self:cr.hArray(properties=self.timeseries_data),
    ...
    def run(self):
        if self.timeseries_data:
            self.fft_data[...].fftcasa(self.timeseries_data[...],1)
#            self.fft_data[...].fftw(self.timeseries_data[...])
        if not self.reference_data:
            self.fft_reference_data.copy(self.fft_data[self.refant])
        else:
            self.fft_reference_data.fftcasa(self.reference_data,1)
        self.fft_data[...].crosscorrelatecomplex(self.fft_reference_data)
        self.crosscorr_data[...].invfftcasa(self.fft_data[...],1)  
#        self.crosscorr_data[...].saveinvfftw(self.fft_data[...],1)  
...
class FitMaxima(tasks.Task):
    ...
    **Usage:**
    ...
    Task=FitMaxima()(data[n_datasets,n_samples],peak_width=20,nsubsamples=16,ncoeffs=5,splineorder=3,doplot=False,plotend=4,plotstart=0,sampleinterval=None,refant=None)
    ...
    **Example:**
    ...
    parameters=dict(
        ...
        xdata=p_(lambda self:cr.hArray(float,[self.dim2,self.peak_width])),
    ...
    def run(self):
        maximum=cr.asvec(self.data[...].maxpos())
        start=cr.Vector(int,copy=maximum)
        end=cr.Vector(int,copy=maximum)
        start-=self.peak_width/2
        end+=self.peak_width/2
        startf=cr.Vector(float,copy=start)
        endf=cr.Vector(float,copy=end)
        self.fits_xdata[...].fillrange(startf,cr.asvec(1.0/self.nsubsamples))
        self.xdata[...].fillrange(start,cr.asvec(1))
        chisquare=self.coeffs[...].bsplinefit(self.covariance[...],self.xpowers[...],self.xdata[...],self.data[...,start:end],startf,endf,cr.asvec(self.splineorder+1))
        self.fits_ydata[...].bsplinecalc(self.fits_xdata[...],self.coeffs[...],startf,endf,cr.asvec(self.splineorder+1))
        self.maxy=self.fits_ydata[...].max()
        self.maxpos=self.fits_ydata[...].maxpos()
        self.maxx=self.fits_xdata[...].elem(self.maxpos)
        if self.doplot:
            self.data[self.plotstart:self.plotend,...].plot()
