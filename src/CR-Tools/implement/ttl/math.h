...
namespace ttl
{
  ...
  /*!
    \brief Returns the natural logarithm of the value if the value
    is larger than zero, otherwise return a low number

    \param begin begin itterator
    \param end end itterator
   */
  template <class T>
    inline T logSave(T val)
    {
      if (val>0) return std::log(val);
      else return LOW_NUMBER;
    }
