...
//================================================================================
//     File was generated from - on Fri Oct 16 15:40:20 CEST 2009
//--------------------------------------------------------------------------------
//
#ifndef HFFUNCS_H
#define HFFUNCS_H
...
class DataFuncLibraryClass {
 public:
  void addFuncnameToLibrary(HString name,HString lib);
  bool isFuncnameInLibrary(HString name, HString lib);
  void addFunc(ConstructorFunctionPointer c_ptr);
  HString findLibrary(HString name, bool fullname=false);

  ObjectFunctionClass* f_ptr(HString name,  HString library="");

  bool inLibrary(HString name,  HString library="");

  DataFuncDescriptor* FuncDescriptor(HString name, HString library="");

  vector<HString> listFunctions(bool doprint=true);
  vector<HString> getFuncnames(HString library, bool doprint=false);
  vector<HString> getLibraries(bool doprint=false);

  DataFuncLibraryClass();
  ~DataFuncLibraryClass();

 private:
  map<HString,vector<HString> > func_library_members;
