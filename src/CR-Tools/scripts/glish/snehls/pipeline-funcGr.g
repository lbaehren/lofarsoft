...
newKretain := function(kretafiles){
  ...
  while ( line1 := read(fd[1]) ) {
    ...
    # flag for used reconstruction, default=T = KASCADE
    erg.flag[fnum] :=T;
    # decision for KASCADE or Grande reconstruction
...
#
#-----------------------------------------------------------------------
pipeSelectEvents := function(infiles, minsize=1, maxsize=1e99, minlmuo=1,maxlmuo=1e99,maxrad=91,extsel=T,maxze=1.6,minze=-1.,totalsizes=F,goodKASCADE=T,minYMD=0,maxYMD=999999){
...
#      reconstar := (inrec.run > 0) & (inrec.age>0.2) & (inrec.age<2.1);
      reconstar := (inrec.run > 0) & (inrec.age>0.4) & (inrec.age<1.8);
     } else { reconstar := T };

    selar := filear & sizesel & radar & zear & YMDar & reconstar & extsel;
    print 'Files:',sum(filear),'SIZE:',sum(sizear),'LMUO:',sum(lmuoar),'Size:',sum(sizesel),'Rad:',sum(radar),'ZE:',sum(zear),'YMD:',sum(YMDar),'reconst',sum(reconstar),'Extern:',sum(extsel),'  Total:',sum(selar)
...
#
#-----------------------------------------------------------------------
pipeSelectEventsKGr := function(infiles, minsize=1, maxsize=1e99, minlmuo=1,maxlmuo=1e99,extsel=T,maxze=1.6,minze=-1.,totalsizes=F,goodKASCADE=T,goodGRANDE=F,minYMD=0,maxYMD=999999,posx=0,posy=0,dlen=1500){
  erg := [=];
  fnum := 1;
  for (file in infiles)  {
    inrec := read_value(file);
   ...
   for (check in 1:len(selar)){
    ...
    lmuoar := inrec.LMUO > minlmuo & inrec.LMUO < maxlmuo;
    zear := inrec.ZE<maxze & inrec.ZE>minze;
    selar  := sizesel & zear &lmuoar;
    selar2 := selar & area & border & reconstar;
