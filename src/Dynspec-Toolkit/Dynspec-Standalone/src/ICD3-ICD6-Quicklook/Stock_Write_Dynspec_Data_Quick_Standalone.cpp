...
/// \file Stock_Write_Dynspec_Data_Quick_Standalone.cpp
///  \brief File C++ (associated to Stock_Write_Dynspec_Data_Quick_Standalone.h) for stocking data parameters, processing the (select or rebin) and write data Matrix in the ICD6's Dynspec Groups 
///  \details  
/// <br /> Overview:
...
using namespace std;
  ...
  Stock_Write_Dynspec_Data_Quick_Standalone::Stock_Write_Dynspec_Data_Quick_Standalone(){}
  Stock_Write_Dynspec_Data_Quick_Standalone::~Stock_Write_Dynspec_Data_Quick_Standalone(){}
  
  void Stock_Write_Dynspec_Data_Quick_Standalone::stockDynspecData(int Ntime,int Nspectral,  int NtimeReal, int NspectralReal)
  { 
  /// <br /> Usage:
  /// <br />   void Stock_Write_Dynspec_Data_Quick_Standalone::stockDynspecData(int Ntime,int Nspectral,  int NtimeReal, int NspectralReal)
  ...
  /// \param  frequencyRebin frequency binning
	    ...
	    int nofTimeToSumEverySecond	(STOCKES.nofSamples().get()/TOTAL_INTEGRATION_TIME*dataTimePercent);
	    if (nofTimeToSumEverySecond==0){nofTimeToSumEverySecond++;}
	    int stepForOneSecond(STOCKES.nofSamples().get()/TOTAL_INTEGRATION_TIME);
      ...
      vector<float> DATA_3DPol(nofTimeToSumEverySecond*CHANNELS_PER_SUBANDS_TEMP*NOF_SUBBANDS_TEMP*4);
      ...
      for (p=0;p<m_Ntime;p++)
	      ...
	      for (int l=0;l<4;l++) 
		{
		    ...
		    for (int r=0;r<nofTimeToSumEverySecond;r++)
			{
			  for (int n=0;n<nofSpectralToSumEverySubband;n++)
			    {		  			  
				if (l==0){DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)]=DATA_2D[(r*m_Nspectral)+n];}
				if (l==1){DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+1]=DATA_2D[(r*m_Nspectral)+n];}
				if (l==2){DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+2]=DATA_2D[(r*m_Nspectral)+n];}
				if (l==3){DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+3]=DATA_2D[(r*m_Nspectral)+n];}
			    }	  
			}
		...
		for (int r=0;r<nofTimeToSumEverySecond;r++)
		  {
		    for (int n=0;n<nofSpectralToSumEverySubband;n++)
		      {		  			  
			  DATA_2D[(r*m_Nspectral)+n]	= (pow(DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)],2) + pow(DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+1],2) + pow(DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+2],2) + pow(DATA_3DPol[obsNofStockes*((m_Nspectral*r)+n)+3],2))/2.0;	
		      }	  
		  }		
