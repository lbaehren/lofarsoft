...
/// \file Stock_Write_Dynspec_Data_Substraction.cpp
///  \brief File C++ (associated to Stock_Write_Dynspec_Data_Substraction_Part.h) for stock process parameters (for rebinning the data)  and write data Matrix for the ICD6's Dynspec Groups 
///  \details  
/// <br /> Overview:
...
using namespace std;
  ...
  Stock_Write_Dynspec_Data_Substraction::Stock_Write_Dynspec_Data_Substraction(){}
  Stock_Write_Dynspec_Data_Substraction::~Stock_Write_Dynspec_Data_Substraction(){}
  
  void Stock_Write_Dynspec_Data_Substraction::stockDynspecData(int Ntime,int Nspectral)
  {
    
  /// <br /> Usage:
  /// <br />     void Stock_Write_Dynspec_Data_Substraction::stockDynspecData(int Ntime,int Nspectral)
  ...
  void Stock_Write_Dynspec_Data_Substraction::writeDynspecData(Group &dynspec_grp,string ID,string file1,string dynspec1,string file2,string dynspec2,float kfactor,string outputFile,File &root_grp,int nbSTOkES,vector<string> stokesComponent,float memoryRAM)  
  {
      
  /// <br /> Usage:
  /// <br />   void Stock_Write_Dynspec_Data_Substraction::writeDynspecData(Group &dynspec_grp,string pathDir,string obsName,string pathFile,string outputFile,File &root_grp,int i,int j,int k,int l,int q,int nbSTOkES,vector<string> stokesComponent,float memoryRAM, int SAPindex)  
  ...
  int nofLastElements((m_Ntime)-((fracTime-1)*sizeTimeLimit));
      ...
      for (p=0;p<fracTime;p++)
	{
	  ...
	  vector<float> DATA_3D_File2(sizeTimeLimit*m_Nspectral*nbSTOkES);
	  vector<float> DATA_3D(sizeTimeLimit*m_Nspectral*nbSTOkES);	  

	  if (p==fracTime-1){if (nofLastElements > 0){DATA_3D_File1.resize(nofLastElements*m_Nspectral*nbSTOkES);}}	  
	  if (p==fracTime-1){if (nofLastElements > 0){DATA_3D_File2.resize(nofLastElements*m_Nspectral*nbSTOkES);}}	  
	  if (p==fracTime-1){if (nofLastElements > 0){DATA_3D.resize(nofLastElements*m_Nspectral*nbSTOkES);}}	  
	  ...
	  // Get matrixes NOW!!
		...
		{ 	  
		  ...
		  STOCKES2.get3D( pos, &DATA_3D_File2[0], size[0],size[1],size[2] );
		  
		  for (i=0;i<sizeTimeLimit*m_Nspectral*nbSTOkES;i++)	    
		    {
		      DATA_3D[i] = DATA_3D_File1[i]-(kfactor*DATA_3D_File2[i]);
		    }		  
	      ...
	      else
		{
		  if (nofLastElements > 0)
			...
			for (i=0;i<nofLastElements*m_Nspectral*nbSTOkES;i++)	    
			  {
			    DATA_3D[i] = DATA_3D_File1[i]-(kfactor*DATA_3D_File2[i]);
			  }		  
