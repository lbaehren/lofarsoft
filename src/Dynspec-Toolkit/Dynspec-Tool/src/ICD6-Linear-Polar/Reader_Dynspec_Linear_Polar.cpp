...
/// \file Reader_Dynspec_Linear_Polar.cpp
///  \brief File C++ (associated to Reader_Dynspec_Linear_Polar.h)for read ICD3 files and put in variables metadatas for the ICD6's Dynspec Group  
///  \details  
/// <br /> Overview:
/// <br /> This function readDynspec is described (programmed) here. It needs as parameter a Stock_Write_Dynspec_Metadata_Linear_Polar object. The readDynspec function will read 
///  dynspec metadatas and stock them in the Stock_Write_Dynspec_Metadata_Linear_Polar object as private attributes thanks to stockDynspecMetadata function.
///  These attributes will be written in the next step by writeDynspecMetadata function which is in the class 
///  Stock_Write_Dynspec_Metadata_Linear_Polar.h  and called at the end of this Reader_Dynspec_Linear_Polar.cpp
...
using namespace std;
  ...
  Reader_Dynspec_Linear_Polar::Reader_Dynspec_Linear_Polar(){}	// constructor 
  Reader_Dynspec_Linear_Polar::~Reader_Dynspec_Linear_Polar(){}	// destructor
...
/// \return boolean value 
  ...
  void Reader_Dynspec_Linear_Polar::readDynspec(string ID, string filename, string dynspec, Stock_Write_Dynspec_Metadata_Linear_Polar *dynspecMetadata,Stock_Write_Dynspec_Data_Linear_Polar *dynspecData,int nbSTOkES,vector<string> stokesComponent)
    {
      
/// <br /> Usage:
/// <br />     void Reader_Dynspec_Linear_Polar::readDynspec(string pathFile,Stock_Write_Dynspec_Metadata_Linear_Polar *dynspecMetadata,Stock_Write_Dynspec_Data_Linear_Polar *dynspecData,int i,int j,int q,int obsNofSAP,int obsNofSource,int obsNofFrequencyBand,int obsNofStockes,vector<string> stokesComponent, int SAPindex)

 
/// \param  pathFile  ICD3 file to read 
/// \param  *dynspecMetadata Reader_Dynspec_Linear_Polar_Part Object for loading dynspec metadata in private attributes
/// \param  *dynspecData Stock_Write_Dynspec_Data_Linear_Polar_Part Object for loading new rebinning for data matrix processing 
...
//   stokesComponent = obj_AXIS_NAMES_POL.value;      
  ...
  string TARGET_DYN(obj_TARGET1.value);
  
  
  Attribute<double> obj_POINT_RA(Dynspec1, "POINT_RA");
  Attribute<double> obj_POINT_DEC(Dynspec1, "POINT_DEC");
  
  Attribute<double> obj_POSITION_OFFSET_RA(Dynspec1, "POSITION_OFFSET_RA");
  Attribute<double> obj_POSITION_OFFSET_DEC(Dynspec1, "POSITION_OFFSET_DEC");
  ...
  double BEAM_NOF_STATIONS(obj_BEAM_NOF_STATIONS1.value);
  vector<string> BEAM_STATIONS_LIST(obj_BEAM_STATIONS_LIST1.value);
  ...
  string PREPROCCESSINGINFO1("Polarization Conversion : I,Q,U,V to I,Lin,PA,TOTAL");
  string PREPROCCESSINGINFO2("initial file = "+filename);
  string PREPROCCESSINGINFO3("Lin = SQRT( Q^2 + U^2)");
  string PREPROCCESSINGINFO4("PA = 0.5 Arctg (U/Q)");
  string PREPROCCESSINGINFO5("TOTAL =SQRT( Q^2 + U^2 + V^2)");
    ...
    }  // end of the Reader_Dynspec_Linear_Polar object 
