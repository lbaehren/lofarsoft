...
#
	...
	DumpUser=${HomeDir:6}
        ...
        export DireName="/data/TBB/$DayTag-$MonthTag-$YearTag/"
        ssh list001 mkdir -p "$DireName"
        export LogDireName="$HOME/logs/$DayTag-$MonthTag-$YearTag/"
		mkdir -p "$LogDireName"
        export DumpLog="$LogDireName"dumplog.txt
...
DumpFun ()					# This function instructs to dump data to CEP
{
        ...
        Time=$(date +%Hh%Mm%Ss%Nns)		 
		DumpTime=$(date)
        echo "---------------------Start dump time:  $Time ------------------"
        Station="$1"
	...
	shift
        ...
        do
	     ...
	     then 
	     Time=$(date +%Hh%Mm%Ss%Nns)		 
	     echo "initiliazing dump at $Time"
             echo "#### tbb2h5-command:" ssh 10.181.0.1 /app64/usg/bin/TBBraw2h5 --ip "$IPaddress" \
		 --port "$Port" --outfile "$FullName" --timeoutRead "$Latency" -F2 -C1 -V
	     nohup /bin/bash -e | ssh 10.181.0.1 /app64/usg/bin/TBBraw2h5 --ip "$IPaddress" \
		 --port "$Port" --outfile "$FullName" --timeoutRead "$Latency" -F2 -C1 -V &
