...
if len(sys.argv) < 5 or len(sys.argv) > 9:
	...
	print "\tbaseline.py <file> <antenna1> <antenna2> " + \
	      ...
	      "['p'hase or 'a'mplitude] [polarization (0-3,4-6)]"
	print "\t<> required"
	print "\t E.g., baseline.py /lifs006/SB3.MS 1 10 0 channel 3 a 6"
...
data_name = "DATA"
#axis_range = [4.10e7, 4.11e7, 0, 1]  # xmin, xmax, ymin, ymax in data coordinates
axis_range = 0     # for default ranges
...
if (range_plot != -1):
	if quantity_plot == 'channel':
		...
		if data_plot == 'a':
			...
			current_value = arctan2((data[pol,range_plot,:]).real,(data[pol,range_plot,:]).imag)%2*pi-pi
			if pol2:  current_value2 = arctan2((data[pol2,range_plot,:]).real,(data[pol2,range_plot,:]).imag)%2*pi-pi
		...
		if data_plot == 'a':
			...
			current_value = arctan2(array((data[pol,:,range_plot]).real),array((data[pol,:,range_plot]).imag))%2*pi-pi
			if pol2: current_value2 = arctan2(array((data[pol2,:,range_plot]).real),array((data[pol2,:,range_plot]).imag))%2*pi-pi
		plot( current_value, "," )
...
# otherwise, plot all channels/times
	...
	if quantity_plot == 'channel':
		...
		for channel in range( nchannels ):
			if data_plot == 'a':
				...
				current_value = arctan2(array((data[pol,channel,:]).real),array((data[pol,channel,:]).imag))%2*pi-pi
				if pol2: current_value2 = arctan2(array((data[pol2,channel,:]).real),array((data[pol2,channel,:]).imag))%2*pi-pi
			plot( current_value, "," )
			if pol2: plot (current_value2, ",")
			if axis_range:  axis(axis_range)
			title("Time vs. Amplitude, Baseline " + \
	...
	if quantity_plot == 'time':
		...
		for t in range( len(time) ):
				...
				current_value = arctan2(array((data[pol,:,t]).real),array((data[pol,:,t]).imag))%2*pi-pi
				if pol2: current_value2 = arctan2(array((data[pol2,:,t]).real),array((data[pol2,:,t]).imag))%2*pi-pi
			plot( current_value, "," )
			if pol2: plot( current_value2, ",")
			if axis_range:  axis(axis_range)
			title("Time vs. Amplitude, Baseline " + \
