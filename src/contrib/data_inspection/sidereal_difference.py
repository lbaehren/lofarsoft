...
# set default values
if len(sys.argv) < 6:  range_plot = -1
...
data_day1 = data[indices_day1,:,:]
time_day2 = time[indices_day1+len(indices_day1)]
data_day2 = data[indices_day1+len(indices_day1),:,:]
...
if (range_plot != -1):
	...
	if data_plot == 'a':
		current_value_day1 = hypot((data_day1[range_plot,:,pol]).real,(data_day1[:,range_plot,pol]).imag)
		if pol2:  current_value2_day1 = hypot((data_day1[:,range_plot,pol2]).real,(data_day1[:,range_plot,pol2]).imag)
		current_value_day2 = hypot((data_day2[:,range_plot,pol]).real,(data_day2[:,range_plot,pol]).imag)
		if pol2:  current_value2_day1 = hypot((data_day2[:,range_plot,pol2]).real,(data_day2[:,range_plot,pol2]).imag)
	elif data_plot == 'p':
		current_value_day1 = arctan2((data_day1[:,range_plot,pol]).imag,(data_day1[:,range_plot,pol]).real)%2*pi-pi
		if pol2:  current_value2_day1 = arctan2((data_day1[:,range_plot,pol2]).imag,(data_day1[:,range_plot,pol2]).real)%2*pi-pi
		current_value_day2 = arctan2((data_day2[:,range_plot,pol]).imag,(data_day2[:,range_plot,pol]).real)%2*pi-pi
		if pol2:  current_value2_day2 = arctan2((data_day2[:,range_plot,pol2]).imag,(data_day2[:,range_plot,pol2]).real)%2*pi-pi
	figure(1)
	subplot(211)
