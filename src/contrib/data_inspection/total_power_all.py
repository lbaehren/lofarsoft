...
for i in antenna_range:
	...
	if quantity_plot == 'channel':
		...
		curve0 = (hypot((data[:,range_plot,0]).real,(data[:,range_plot,0]).imag))[antenna_indices]
		curve1 = (hypot((data[:,range_plot,1]).real,(data[:,range_plot,1]).imag))[antenna_indices]
		curve2 = (hypot((data[:,range_plot,2]).real,(data[:,range_plot,2]).imag))[antenna_indices]
		curve3 = (hypot((data[:,range_plot,3]).real,(data[:,range_plot,3]).imag))[antenna_indices]
	...
	#  plot autocorrelations for one time
		...
		curve0 = hypot((antenna_data[:,range_plot,0]).real,(antenna_data[:,range_plot,0]).imag)
		curve1 = hypot((antenna_data[:,range_plot,1]).real,(antenna_data[:,range_plot,1]).imag)
		curve2 = hypot((antenna_data[:,range_plot,2]).real,(antenna_data[:,range_plot,2]).imag)
		curve3 = hypot((antenna_data[:,range_plot,3]).real,(antenna_data[:,range_plot,3]).imag)
		if len(antenna_range) == 1:  ax = fig.add_subplot(1,1,1)
		elif len(antenna_range) > 1:  ax = fig.add_subplot(plots_per_page/2,2,antenna_range.index(i)%plots_per_page+1)
