...
import time, os, select, signal, pty, fcntl, sys, message
...
class PipelineCook(WSRTCook):
    def __init__(self, task, inputs, outputs, messages):
        WSRTCook.__init__(self, task, inputs, outputs, messages)
        try:
            exec(eval("'from %s import %s' % (task, task)"))
            exec(eval("'self.recipe = %s()' % task"))
        except Exception, e:
            self.messages.append(ErrorLevel, 'Exception caught: ' + str(type(Exception)) + ' ' + str(e))
            self.recipe = None
    ...
    def try_running(self):
        ...
        self.recipe.messages = self.messages
        if self.recipe.run(self.task):
            self.copy_outputs()
        else:
    ...
    def copy_inputs(self):
        for k in self.inputs.keys():
            if k in self.recipe.inputs.keys():
                self.recipe.inputs[k] = self.inputs[k]
            else:
                self.messages.append(DebugLevel, self.task + ' has no argument ' + str(k))
...
class SystemCook(WSRTCook):
    ...
    def __init__(self, task, inputs, outputs, messages):
        ...
        ## We can only have a pipe for stderr as otherwise stdio changes it's buffering strategy
...
##        self.poll = select.poll()
    ...
    def spawn(self, env=None):
        ...
        try:
            ...
            self.messages.append(ErrorLevel, 'Unable to fork:' + str(e))
            raise CookError ('fork failed')
        if self._pid == 0: ## the new client
            try:
                #fcntl.fcntl(self.errw, fcntl.F_SETFL, os.O_NONBLOCK)
                #os.dup2(self.outputpipe_front, 1) ## This makes stdio screw up buffering because a pipe is a block device
            ...
            if text:
                ...
                for line in text.split('\n'): ## still have odd behaviour for gear output
                    if file == self._child_fd:
                        self.messages.append(VerboseLevel, self.StripNoPrint(line))
                    elif file == self._errorpipe_end:
                        self.messages.append(NotifyLevel, self.StripNoPrint(line))
            else:
                if file == self._child_fd:
                    self._fd_eof   = 1
                elif file == self._errorpipe_end:
...
##        return 1 ##else
  ...
  ##        self.poll.unregister(self._errorpipe_end)
          ...
          if (self.exitstatus == 0 or self.exitstatus > 255):
              ...
              self.messages.append(VerboseLevel, self.task + ' has ended with exitstatus: ' + str(self.exitstatus))
          else:
              self.messages.append(NotifyLevel, self.task + ' was aborted with exitstatus: ' + str(self.exitstatus))
        except Exception, e:
          self.messages.append(ErrorLevel, 'Exception caught: ' + str(type(Exception)) + ' ' + str(e))
...
class MiriadCook(SystemCook):
    def __init__(self, task, inputs, outputs, messages):
        ...
        SystemCook.__init__(self, mirbin + task, inputs, outputs, messages)
        self.messages.append(DebugLevel, 'Using ' + task + ' found in ' + mirbin)

class AIPSCook(SystemCook):
    def __init__(self, task, inputs, outputs, messages):
        # someting with the Parseltongue AIPSTask
        SystemCook.__init__(self, task, inputs, outputs, messages)
...
class GlishCook(SystemCook):
    def __init__(self, task, inputs, outputs, messages):
        ...
        SystemCook.__init__(self, task, inputs, outputs, messages)
        self.messages.append(DebugLevel, 'Using ' + task + ' with AIPSPATH ' + aipspath)
