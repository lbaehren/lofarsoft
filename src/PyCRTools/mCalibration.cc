...
double interpolate_amplitude_trilinear(const std::complex<double> (&V)[8],
                                       ...
                                       const double x1, const double y1, const double z1)
{
    double c00, c10, c01, c11, c0, c1, rho;
    double a[8];
    int i;
...
double interpolate_phase_trilinear(const std::complex<double> (&V)[8],
                                   ...
                                   const double x1, const double y1, const double z1)
{
    double c00, c10, c01, c11, c0, c1, theta;
    double a[8];
    int i;
    ...
    const HNumber pstart, const HNumber pstep, const HInteger pn)
{
  ...
  *(J_it+0) = interpolate_trilinear(V, x, y, z_xdipole, x0, y0, z0, x1, y1, z1);
  ...
  // Interpolate to find Jones matrix component for theta
  *(J_it+2) = -1 * interpolate_trilinear(V, x, y, z_ydipole, x0, y0, z0, x1, y1, z1);
  ...
  *(J_it+1) = -1 * interpolate_trilinear(V, x, y, z_xdipole, x0, y0, z0, x1, y1, z1);
...
//-----------------------------------------------------------------------
#define HFPP_FUNCDEF  (HFPP_VOID)(HFPP_FUNC_NAME)("$DOCSTRING")(HFPP_PAR_IS_SCALAR)()(HFPP_PASS_AS_VALUE)
#define HFPP_PARDEF_0 (HInteger)(nof)()("Number of consectutive zeros.")(HFPP_PAR_IS_VECTOR)(STDIT)(HFPP_PASS_AS_REFERENCE)
