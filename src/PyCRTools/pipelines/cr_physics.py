    ...
    for station in stations:
        ...
        with process_station(station) as station:
            ...
            if options.debug:
                ...
                t = np.arange(pulse_end - pulse_start) / f["CLOCK_FREQUENCY"]
...
#        print noGlitchIndices
        try:
            ...
            wavefront = cr.trun("Wavefront", arrivaltimes=all_station_pulse_delays[noGlitchIndices], positions=all_station_antenna_positions[noGlitchIndices], stationnames=all_station_antennas_stationnames[noGlitchIndices], loracore=core, lora_direction=lora_direction, eventID = options.id, bruteforce_fit = bruteforce_fit, use_colormap="hot_r", use_title=False, save_plots=True, plot_prefix=event_plot_prefix,plot_type=options.plot_type, plotlist=event["crp_plotfiles"])
