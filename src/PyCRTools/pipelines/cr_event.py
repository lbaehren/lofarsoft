...
def run_pipeline_event(event_info, dump_html=False):
    print "Running pipeline_event..."        # DEBUG

    for datafile_info in event_info.datafiles:
        full_filename = os.path.join(db_manager.settings.datapath, datafile_info.filename)
        print "Processing file: %s" %(full_filename)

        # Get station info object
        if datafile_info.stations:
            station_info = datafile_info.stations[0] # 1st element should be the one to process
        else:
            raise ValueError("No stations in datafile_info object.")


        for current_polarization in polarizations:
            # Get polarization info object
            if station_info.polarization:
                polarization_info = station_info.polarization["%d"%(current_polarization)]
            else:
                raise ValueError("No polarizations in station_info object.")
            polarization_info.status="PROCESSING"
            polarization_info.write(recursive=False)
            ...
            try:
                ...
                datafile_info.write(recursive=False)
                write_results(polarization_info, results)
                #xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
                if dump_html:
                    finish_file()
            ...
            if "HBA" in datafile["ANTENNA_SET"]:
                polarization_info.status = "HBA SKIPPED"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
                if dump_html:
                    finish_file()
            ...
            if max_data_length>0 and max(datafile["DATA_LENGTH"])>max_data_length:
                print "ERROR: Data file size is too large (",max(datafile["DATA_LENGTH"]),") - skipping this file!"
                polarization_info.status = "DATA_LENGTH BAD"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)

            if min(datafile["DATA_LENGTH"])<min_data_length or min(datafile["DATA_LENGTH"]) < blocksize:
                print "ERROR: Data file size is too small (",max(datafile["DATA_LENGTH"]),") - skipping this file!"
                polarization_info.status = "FILE TOO SMALL"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
            ...
            if os.path.exists(lora_logfile):
                ...
                if tbb_starttime_sec:
                    try:
                        ...
                        print "#ERROR - LORA trigger information not found"
                        polarization_info.status = "TRIGGER TOO LATE"
                        polarization_info.write(recursive=False)
                        statuslist.append(polarization_info.status)
                        #xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                        write_results(polarization_info, results)
                    ...
                    polarization_info.status = "TIMESTAMP NOT IN LORA LOGFILE"
                    polarization_info.write(recursive=False)
                    statuslist.append(polarization_info.status)
                    # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                    write_results(polarization_info, results)
    ...
    #            DirectionFitTriangles = dict(maxiter=2,rmsfactor=0,minrmsfactor=0,direction_guess=lora_direction,direction_guess_label="LORA"), # determine delays iteratively
            ...
            if ndipoles<minimum_number_good_antennas:
                print "#ERROR: To few good antennas ("+str(ndipoles)+")"
                polarization_info.status = "TOO FEW ANTENNAS"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
    ...
    #        cabledelays=hArray(cabledelays)
            ...
            try:
                ...
                print "Error reading file - skipping this file"
                polarization_info.status = "READ ERROR"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
            ...
            if pulse_npeaks==0:
                ...
                print "************************************************************************"
                print "ERROR: LocatePulseTrain: No pulses found!"
                polarization_info.status = "NO PULSE"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
    ...
    #        maxima_power.maxx /= 16.0
            ...
            if nantennas_with_strong_pulses<min_number_of_antennas_with_pulses:
                print "ERROR: LocatePulseTrain: Not enough pulses found for beam forming!"
                polarization_info.status = "TOO FEW PULSES"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
    ...
    #        delays=hArray(copy=cabledelays)
            ...
            direction=trerun("DirectionFitTriangles","direction",pardict=par,positions=good_pulse_antenna_positions,timelags=good_pulse_lags, cabledelays=good_pulse_cabledelays, verbose=True,doplot=True)
              # put in zeros for 'cable delays' as they are fitted and put into the same array...
            if direction.ngood == 0:
                polarization_info.status = "NO GOOD TRIANGLES FOUND"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
    ...
    #        time_lags -= cabledelays
            ...
            if maximum < 0 or maximum >= blocksize:
                print "FITTED MAXIMUM OUTSIDE RANGE"
                polarization_info.status = "FITTED MAXIMUM OUTSIDE RANGE"
                polarization_info.write(recursive=False)
                statuslist.append(polarization_info.status)
                # xmldict.dump(os.path.join(outputdir_with_subdirectories,"results.xml"), results)
                write_results(polarization_info, results)
            ...
            if (event_info.status != "CR_FOUND"):
                if (final_status == "OK"):
                    event_info.status = "CR_FOUND"
                else:
                    event_info.status = "CR_NOT_FOUND"
                event_info.write(recursive=False)
...
def write_results(polarization_info, results):
    if results and polarization_info:
...
def bootstrap_old():
    ...
    ########################################################################
    # Update the database when processing files.
    ########################################################################

    if not args:
        filefilter = []             # Use db, not datafile
        if (options.event_id > 0):
            pass
        else:
    ...
    if not db:
        raise ValueError("No valid database was set")
    ...
    for full_filename in files:
        run_pipeline(full_filename)


def bootstrap_new():

    if filefilter:
        print "Running over all files (db will NOT be updated)..."
        # This will run the original pipeline which is located in run_pipeline_datafile()

        filenames = args
        for filename in filenames:
            full_filename = os.path.join(datadir, filename)
            print "  processing %s..." %(full_filename)
            run_pipeline_datafile(full_filename)

    else:
        print "Running over event_id = %d..." %(event_id)
        if event_id > 0:
            event_info = db_manager.getEvent(id=event_id)
            run_pipeline_event(event_info)
            event_info.summary()
        else:
