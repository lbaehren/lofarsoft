...
topdir = '/Users/acorstanje/triggering/CR/results_withcaltables'
event = ''#'VHECR_LORA-20111228T200122.223Z'
...
def gatherresults(topdir, event, antennaSet):
    """
    ...
    if event:
        eventlist = [os.path.join(topdir, event)]
    else:
    ...
    #                print '*** ERROR: WRONG NUMBER OF ANTENNAS!'
        ...
        allEvents.append(thisEventsArrivalTimes)
...
#            timestamps.extend(timelist)
    ...
    for id in antid:  # group by stations
    ...
    for station in antennasPerStationID:
        ...
        for thisEventsArrivalTimes in allEvents: 
            ...
            for id in theseEvenIDs:
                ...
                if thisRCU + 1 in theseRCUnumbers: # cannot assume id -> id+1 for corresponding odd pol.
                    ...
                    oddID = oddID[0]
                    if id in thisEventsArrivalTimes.keys() and oddID in thisEventsArrivalTimes.keys():
...
gatherresults(topdir, event, antennaset)
