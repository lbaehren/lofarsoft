...
event = '/Users/acorstanje/triggering/CR/results_withcaltables/VHECR_LORA-20111228T200122.223Z/'
...
fptask = trerun("plotfootprint", "0", colormap = 'jet', filefilter = event, pol=polarization, plotlora=False) # no parameters set...

times = fptask.arrivaltime.toNumpy()
plotsizes = hArray(dimensions = fptask.arrivaltime, fill=50.0)
fptask = trerun("plotfootprint", "0", colormap = 'jet', filefilter = event, power = plotsizes, pol=polarization, plotlora=False)  # want to see all dots the same size, not proportional to power in this case.
...
plt.figure()
fptask = trerun("plotfootprint", "1", colormap = 'jet', filefilter = event, arrivaltime=hArray(residu), power = plotsizes, pol=polarization, plotlora=False) 
plt.figure()
# plot residuPlanar = data times - bestfit planewave times
fptask = trerun("plotfootprint", "2", colormap = 'jet', filefilter = event, arrivaltime=hArray(residuPlanar), power = plotsizes, pol=polarization, plotlora=False) 
plt.figure()
# plot residu - residuPlanar; show curvature from the best-fitted function
fptask = trerun("plotfootprint", "3", colormap = 'jet', filefilter = event, arrivaltime=hArray(1.0e9*bestfitDelays - 1.0e9*bestfitDelaysPlanar), power = plotsizes, pol=polarization, plotlora = False)
