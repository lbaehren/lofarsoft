...
def geomagneticAngle(az, el, time, lon = 6.869837540, lat = 52.915122495, height = 5.0, filename=None):
    ...
    *time* date of observation as Python date object
    *az* azimuth (Eastward positive from North)
    *el* elevation (Positive up from horizon)
    *lon* WGS84 longitude
    *lat* WGS84 latitude
    *height* height above mean sea level in meters
    *filename* name of file with model coefficients
    """

    # Get filename of file with model coefficients from LUS installation
    if not filename:
        filename = os.path.join(os.environ["LOFARSOFT"], "src/PyCRTools/extras/WMM.COF")
    ...
    if isinstance(time, date):
        ...
        raise ValueError("Time is not a valid datetime.date object or UNIX timestamp.")

    # Construct object with implementation of WMM model
    gm = GeoMag(filename)
