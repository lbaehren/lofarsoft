...
# Create a list of all the parameters with default values (and initialization functions)
    ...
    def __setitem__(self, par, val):
        ...
        if par not in self.parameters + self.auxparameters:
            self.auxparameters.append(par)
        if not self.isLocal(par):
            self.locals.add(par)
    ...
    def markLocal(self, par):
        if not self.isLocal(par):
            self.locals.add(par)  # Make parameter local
...
# other). The attribute will be extended automatically.
    ...
    def default_numin_i(self):
        ...
        if self["numin"] > 0:
            return self["frequency"].findlowerbound(self["numin"]).val()
        else:
            return 1
    ...
    def default_numax_i(self):
        """Channel number in spectrum of the maximum frequency of the useable bandwidth. Negative if to be ignored."""
        if self["numax"] > 0:
            return self["frequency"].findlowerbound(self["numax"]).val()
        else:
...
# Donwsample spectrum
    if ws["numin"] > 0:
        ws["numin_i"] = frequency.findlowerbound(ws["numin"]).val()
    else:
        ws["numin_i"] = 1
    if ws["numax"] > 0:
        ws["numax_i"] = frequency.findlowerbound(ws["numax"]).val()
    else:
        ws["numax_i"] = len(frequency)
...
#
    if ws["verbose"]:
        print time.clock() - ws["t0"], "s: Fitting baseline."
    if ws["fittype"] == "POLY":
        if ws["verbose"]:
...
#    meanrms.sqrt()
    if ws["verbose"]:
        print time.clock() - ws["t0"], "s: Done fitting baseline."
    if ws["doplot"]:
...
def CheckParameterConformance(data, keys, limits):
    ...
    result = []
    for k in keys:
        if (data[keys[k]] < limits[k][0]) | (data[keys[k]] > limits[k][1]):
            result.append(k)
    return result
...
# Start checking
    if verbose:
        if not datafile == None:
            print "Quality checking of file ", datafile["FILENAME"]
