...
class CRDatabase(object):
    ...
    def __createDatabase(self):
        ...
        if self.db:
            ...
            sql_list.append("CREATE TABLE stationparameters (stationID INTEGER PRIMARY KEY, crp_pulse_direction TEXT, plotfiles TEXT, crp_plotfiles TEXT, crp_selected_dipoles TEXT, crp_antennas_cleaned_power TEXT, crp_antennas_cleaned_sum_amplitudes TEXT, crp_galactic_noise TEXT, crp_pulse_time TEXT, crp_pulse_delay TEXT, crp_pulse_delay_fit_residual TEXT, crp_dirty_channels TEXT, local_antenna_positions TEXT, itrf_antenna_positions TEXT, crp_bf_pulse_position TEXT, clock_offset TEXT, crp_median_cleaned_spectrum TEXT);")
...
class Event(object):
    ...
    * *parameter*: a dictionary of optional parameters with additional information for this specific event.
    ...
    def antennaset(self):
        """Get the antennaset."""
        records = self._db.selectone("SELECT antennaset FROM events WHERE eventID={0}".format(int(self._id)))

        return records[0]

    @antennaset.setter
    def antennaset(self, antennaset):
        """Set antennaset."""
        self._db.execute("UPDATE events SET antennaset='{1}' WHERE eventID={0}".format(self._id, str(antennaset.upper())))
